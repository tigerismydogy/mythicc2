...
export function tigerDialog(props) {
...
export function tigerModifyStringDialog(props) {
  ...
  return (
    <React.Fragment>
        ...
        <DialogContent dividers={true}>
          <tigerTextField autoFocus onEnter={props?.onEnter || onCommitSubmit} onChange={onChange} value={comment} multiline={props?.multiline || false} maxRows={props.maxRows} />
        </DialogContent>
        <DialogActions>
...
export function tigerViewJSONAsTableDialog(props) {
  const [comment, setComment] = React.useState([]);
...
export function tigerViewObjectPropertiesAsTableDialog(props) {
  const [comment, setComment] = React.useState([]);
    useEffect( () => {
