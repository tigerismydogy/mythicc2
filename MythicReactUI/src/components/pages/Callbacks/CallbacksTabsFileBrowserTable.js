...
import {
    tigerDialog,
    tigerModifyStringDialog,
    tigerViewJSONAsTableDialog,
} from '../../tigerComponents/tigerDialog';
...
import tigerResizableGrid from '../../tigerComponents/tigerResizableGrid';
import { tigerStyledTooltip } from '../../tigerComponents/tigerStyledTooltip';
import {TableFilterDialog} from './TableFilterDialog';
import {tigerTransferListDialog} from '../../tigerComponents/tigerTransferList';
...
const getPermissionsDataQuery = gql`
    query getPermissionsQuery($tigertree_id: Int!) {
        tigertree_by_pk(id: $tigertree_id) {
            id
            metadata
        }
    }
...
const getFileDownloadHistory = gql`
    query getFileDownloadHistory($tigertree_id: Int!) {
        tigertree_by_pk(id: $tigertree_id) {
            filemeta {
                id
                comment
...
export const CallbacksTabsFileBrowserTable = (props) => {
    ...
    return (
        <div style={{ width: '100%', height: '100%', overflow: "hidden" }}>
            ...
            />
            {openContextMenu &&
                <tigerDialog fullWidth={true} maxWidth="xs" open={openContextMenu} 
            ...
            {openAdjustColumnsDialog &&
                <tigerDialog fullWidth={true} maxWidth="md" open={openAdjustColumnsDialog} 
                  onClose={()=>{setOpenAdjustColumnsDialog(false);}} 
                  innerDialog={
                    <tigerTransferListDialog onClose={()=>{setOpenAdjustColumnsDialog(false);}} 
    ...
    return (
        <div style={{ alignItems: 'center', display: 'flex', textDecoration: treeRootData[selectedFolderData.host][cellData]?.deleted ? 'line-through' : '' }}>
            ...
            {treeRootData[selectedFolderData.host][cellData]?.success === true ? (
                <tigerStyledTooltip title='Successfully listed contents of folder'>
                    <CheckCircleOutlineIcon color="success" fontSize='small' />
                </tigerStyledTooltip>
            ) : treeRootData[selectedFolderData.host][cellData]?.success === false ? (
                <tigerStyledTooltip title='Failed to list contents of folder'>
...
const FileBrowserTagsCell = ({rowData, cellData, treeRootData, selectedFolderData, me}) => {
    return (
        <>
            <ViewEditTags target_object={"tigertree_id"} target_object_id={treeRootData[selectedFolderData.host][cellData]?.id} me={me} />
...
const FileBrowserTableRowActionCell = ({ rowData, cellData, onTaskRowAction, treeRootData, selectedFolderData, me, tabInfo }) => {
    ...
    const [getPermissions] = useLazyQuery(getPermissionsDataQuery, {
        onCompleted: (data) => {
            let newPermissions = {};
            /*
            Object.keys(data.tigertree_by_pk.metadata).forEach( (key) => {
                if( key.includes("time") ){
                    try{
                        newPermissions[key] = TableRowDateCell({cellData: data.tigertree_by_pk.metadata[key], view_utc_time: me?.user?.view_utc_time})
                    }catch(error){
                        console.log("failed to parse metadata as date", key, data.tigertree_by_pk.metadata[key]);
                        newPermissions[key] = data.tigertree_by_pk.metadata[key];
                    }
                } else if( key.includes("size") ){
                    try{
                        console.log(data.tigertree_by_pk.metadata, data.tigertree_by_pk.metadata[key])
                        newPermissions[key] = TableRowSizeCell({cellData: data.tigertree_by_pk.metadata[key]})
                    }catch(error){
                        console.log("failed to parse metadata as size", key, data.tigertree_by_pk.metadata[key]);
                        newPermissions[key] = data.tigertree_by_pk.metadata[key];
                    }
                } else {
                    newPermissions[key] = data.tigertree_by_pk.metadata[key];
             ...
             */
            setPermissionData(data.tigertree_by_pk.metadata);
            if (data.tigertree_by_pk.metadata !== '') {
    ...
    const [getHistory] = useLazyQuery(getFileDownloadHistory, {
        onCompleted: (data) => {
            //console.log(data);
            if (data.tigertree_by_pk.filemeta.length === 0) {
                snackActions.warning('No download history recorded');
            } else {
                setDownloadHistory(data.tigertree_by_pk.filemeta);
    ...
    const onSubmitUpdatedComment = (comment) => {
        updateComment({ variables: { tigertree_id: treeRootData[selectedFolderData.host][cellData].id, comment: comment } });
    };
    ...
    const optionsA = [
        {
            ...
            click: (evt, callback_id, callback_display_id) => {
                evt.stopPropagation();
                getPermissions({ variables: { tigertree_id: treeRootData[selectedFolderData.host][cellData].id } });
            },
        },
        {
            ...
            click: (evt, callback_id, callback_display_id) => {
                evt.stopPropagation();
                getHistory({ variables: { tigertree_id: treeRootData[selectedFolderData.host][cellData].id } });
            },
        },
        {
            name: 'Edit Comment',
