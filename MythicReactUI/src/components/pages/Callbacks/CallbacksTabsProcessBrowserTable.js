...
import { tigerDialog, tigerViewJSONAsTableDialog, tigerModifyStringDialog } from '../../tigerComponents/tigerDialog';
...
const FileBrowserTableRowActionCell = ({rowData, onTaskRowAction, treeRootData, host, viewSingleTreeData,setSingleTree, toggleViewSingleTreeData, tabInfo}) => {
    ...
    const onSubmitUpdatedComment = (comment) => {
        updateComment({ variables: { tigertree_id: treeRootData[host][rowData["full_path_text"] /*+ uniqueSplitString + rowData["callback_id"]*/].id, comment: comment } });
    };
    ...
    const optionsA = [
        {
            ...
            evt.stopPropagation();
            getPermissions({variables: {tigertree_id: rowData.id}});
        }},
        {
            name: 'Edit Comment',
    ...
    return (
        ...
        <React.Fragment>
            ...
            {viewPermissionsDialogOpen &&
                <tigerDialog fullWidth={true} maxWidth="md" open={viewPermissionsDialogOpen} 
                    onClose={()=>{setViewPermissionsDialogOpen(false);}} 
                    innerDialog={<tigerViewJSONAsTableDialog title="View Detailed Data" leftColumn="Attribute" 
