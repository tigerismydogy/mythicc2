...
import {tigerDialog, tigerViewJSONAsTableDialog} from "../../tigerComponents/tigerDialog";
import {tigerSelectFromListDialog} from "../../tigerComponents/tigerSelectFromListDialog";
...
import {tigerDisplayTextDialog} from "../../tigerComponents/tigerDisplayTextDialog";
                    ...
                    if(e.source.id === node.id || e.destination.id === node.id){
                        if(e.end_timestamp === null){
                            if(e.source.id === e.destination.id){
                                return [...prev, {...e, "display": e.source.display_id + " --> " + e.c2profile.name + " --> tiger"}];
                            } else {
    ...
    <>
        ...
        <DialogContent style={{height: "calc(70vh)" }}>
            {manuallyRemoveEdgeDialogOpen &&
                <tigerDialog fullWidth={true} maxWidth="sm" open={manuallyRemoveEdgeDialogOpen}
                              onClose={()=>{setManuallyRemoveEdgeDialogOpen(false);}}
                              innerDialog={<tigerSelectFromListDialog onClose={()=>{setManuallyRemoveEdgeDialogOpen(false);}} identifier="id" display="display"
            ...
            {manuallyAddEdgeDialogOpen &&
                <tigerDialog fullWidth={true} maxWidth="sm" open={manuallyAddEdgeDialogOpen}
            ...
            {openSelectLinkCommandDialog &&
                <tigerDialog fullWidth={true} maxWidth="sm" open={openSelectLinkCommandDialog}
                              onClose={()=>{setOpenSelectLinkCommandDialog(false);}}
                              innerDialog={<tigerSelectFromListDialog onClose={()=>{setOpenSelectLinkCommandDialog(false);}}
...
export const DrawC2PathElementsFlow = ({edges, panel, view_config, theme, contextMenu, providedNodes}) =>{
    ...
    React.useEffect( () => {
        let tempNodes = [{
            ...
            height: 100,
            width: 50,
            data: {label: "tiger", img: "/static/tiger.svg", istiger: true}
        }];
        let tempEdges = [];
        let parentNodes = [];

        const add_edge_to_tiger = (edge, view_config) => {
            ...
            for(let i = 0; i < tempEdges.length; i++){
                if(tempEdges[i].id === edgeID){
                    found = true;
                    if(edge.id >= tempEdges[i].tiger_id){
            ...
            /*
            g.setEdge(edge.source.id, "tiger",  {label: edge.c2profile.name, edge_id: edge.id, end_timestamp: edge.end_timestamp,
        ...
        const createNewEdges = () => {
            // loop through until all edges have one side marked as "toward_tiger"
            let edgesToUpdate = edges.length;
            if (edgesToUpdate === 0) {return []}
            let edgesUpdated = 0;
            let tempNewEdges = [...edges];
            let totigerIds = new Set();
            let loop_count = 0;
            while(edgesUpdated < edgesToUpdate){
                ...
                tempNewEdges = tempNewEdges.map( e => {
                    //console.log(e)
                    if(!e.source.to_tiger && !e.destination.to_tiger){
                        if(e.source.id === e.destination.id){
                            e.source.to_tiger = true;
                            e.destination.to_tiger = true;
                            totigerIds.add(e.source.id);

                            edgesUpdated += 1;
                        } else if(totigerIds.has(e.source.id)){
                            e.source.to_tiger = true;
                            e.destination.to_tiger = false;
                            edgesUpdated += 1;
                        } else if(totigerIds.has(e.destination.id)){
                            e.destination.to_tiger = true;
                            e.source.to_tiger = false;
                            edgesUpdated += 1;
                        } else {
                            ...
                            tempNewEdges.forEach( e2 => {
                                if(e2.source.id === e.source.id){
                                    ...
                                    if(e2.destination.to_tiger){
                                        e.source.to_tiger = true;
                                        edgesUpdated += 1;
                                    }
                                } else if(e2.destination.id === e.source.id){
                                    if(e2.source.to_tiger){
                                        e.source.to_tiger = true;
                                        edgesUpdated += 1;
                                    }
                                }
        ...
        // need to add fake edges between parent groups and tiger so that rendering will be preserved
        updatedEdges.forEach( (edge) => {
            if(!view_config["include_disconnected"] && edge.end_timestamp !== null){return}
            if(edge.destination.id === edge.source.id){
                if(hasEdge(edge.source.id, "tiger", edge.c2profile.name)){
                    // we already have an edge to tiger from our source id, check if this edge is newer or not
                    if(edge.id > getEdge(edge.source.id, "tiger", edge.c2profile.name).tiger_id){
                        add_edge_to_tiger(edge, view_config);
                    }
                }else{
                    //this is a new edge to tiger
                    add_edge_to_tiger(edge, view_config);
                }
            }else{
                ...
                if(view_config["packet_flow_view"]){
                    ...
                    if(hasEdge(destination_str_id, source_str_id, edge.c2profile.name)){
                        ...
                        if(edge.id > getEdge(destination_str_id, source_str_id, edge.c2profile.name).tiger_id){
                    ...
                    if(hasEdge(source_str_id, destination_str_id, edge.c2profile.name)){
                        //we've seen an edge between these two before
                        if(edge.id > getEdge(source_str_id, destination_str_id, edge.c2profile.name).tiger_id){
                            add_edge_p2p(edge, view_config);
                        }else{
                            //console.log("doing nothing, dropping data");
                        }
        ...
        if(shouldUseGroups(view_config)){
            // only add in edges from parents to parents/tiger if we're doing egress flow
            for(let i = 0; i < parentNodes.length; i++){
                // every parentNode needs a connection to _something_ - either to tiger or another parentNode
                for(let j = 0; j < tempEdges.length; j++){
                    ...
                    if(tempEdges[j].data.source.parentNode === parentNodes[i].id){
                        ...
                        } else if(tempEdges[j].data.target.parentNode === "tiger") {
                            //console.log("fake edge is for tiger, and one exists already - update it")
    ...
    return (
        <div style={{height: "100%", width: "100%", overflow: "hidden"}} ref={viewportRef}>
            ...
            {openDictionaryButton &&
                <tigerDialog fullWidth={true} maxWidth="lg" open={openDictionaryButton}
                              onClose={finishedTasking}
                              innerDialog={<tigerViewJSONAsTableDialog title={taskingData.title} leftColumn={taskingData.leftColumnTitle}
                                                                        rightColumn={taskingData.rightColumnTitle} value={taskingData.value} onClose={finishedTasking} />}
                />
            }
            {openStringButton &&
                <tigerDisplayTextDialog fullWidth={true} maxWidth="lg" open={openStringButton} title={taskingData?.title || "Title Here"} value={taskingData?.value || ""}
                                         onClose={finishedTasking}
                />
            }
            {openTableButton &&
                <tigerDialog fullWidth={true} maxWidth="xl" open={openTableButton}
                              onClose={finishedTasking}
