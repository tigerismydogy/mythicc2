...
import {ModifyCallbacktigerTreeGroupsDialog} from "./ModifyCallbacktigerTreeGroupsDialog";
...
export const CallbacksTableIDCell = React.memo(({rowData, toggleLock, updateDescription, setOpenHideMultipleDialog, setOpenTaskMultipleDialog}) =>{
    ...
    const [openEdittigerTreeGroupsDialog, setOpenEdittigerTreeGroupsDialog] = React.useState(false);
    ...
    const options =  [
        ...
        {name: "Modify Groupings", icon: <WidgetsIcon style={{paddingRight: "5px"}} />, click: (evt) => {
            evt.stopPropagation();
            setOpenEdittigerTreeGroupsDialog(true);
        }}
    ];
    return (
        <div>
            ...
            {openEditDescriptionDialog &&
                <tigerDialog 
                    ...
                    innerDialog={
                        <tigerModifyStringDialog title={"Edit Callback's Description"} 
                            onClose={() => {setOpenEditDescriptionDialog(false);}} 
            ...
            {openEdittigerTreeGroupsDialog &&
                <tigerDialog
                    fullWidth={true}
                    maxWidth={"lg"}
                    open={openEdittigerTreeGroupsDialog}
                    onClose={() => {setOpenEdittigerTreeGroupsDialog(false);}}
                    innerDialog={
                        <ModifyCallbacktigerTreeGroupsDialog callback_id={rowDataStatic.id}
                            onClose={() => {setOpenEdittigerTreeGroupsDialog(false);}} />
                    }
                />
            }
...
export const CallbacksTablePayloadTypeCell = React.memo( ({rowData}) => {
    return (
        <tigerStyledTooltip title={rowData?.payload?.payloadtype?.name}>
            <img
    ...
    return (
        <>
            <div style={{display: "flex", alignItems: "center"}}>
                {options.length > 1 &&
                    <tigerStyledTooltip title={"Adjust Displayed"}>
            ...
            {openPickIP && 
                <tigerDialog fullWidth={true} open={openPickIP}  onClose={() => {setOpenPickIP(false);}}
                innerDialog={
                    <tigerSelectFromRawListDialog 
...
export const CallbacksTableSleepCell = React.memo( ({rowData, cellData, updateSleepInfo}) => {
    ...
    return (
        <div>
            ...
            />
            { openSleepDialog &&
                <tigerDialog fullWidth={true} open={openSleepDialog}  onClose={() => {setOpenSleepDialog(false);}}
                    innerDialog={
                        <tigerModifyStringDialog title={"View Sleep Information"} multiline maxRows={10}
