...
import logo from '../../../assets/tiger@2x.png';
    ...
    const submit = e => {
        ...
        fetch('/auth', requestOptions).then((response) => {
            if(response.status !== 200){
                snackActions.warning("HTTP " + response.status + " Error: Check tiger logs");
                return;
            }
            ...
            me.loggedIn ?
               ...
               : (
                    <div style={{backgroundColor: "transparent"}}>
                        <CardContent>
                            <img src={logo} height="400px" alt="tiger logo"/>
                            <form onSubmit={submit}>
                                <tigerTextField name='username' value={username} onChange={onUsernameChange} width={30} />
                                <tigerTextField name='password' type="password" onEnter={submit} value={password} onChange={onPasswordChange} width={30} />
