...
import {tigerSelectFromRawListDialog} from '../../tigerComponents/tigerSelectFromListDialog';
...
function CredentialTableRow(props){
    ...
    return (
        <React.Fragment>
            <TableRow hover>
                <tigerConfirmDialog onClose={() => {setOpenDeleteDialog(false);}} onSubmit={onAcceptDelete} open={openDeleteDialog} acceptText={props.deleted ? "Restore" : "Remove" }/>
                {editCommentDialogOpen &&
                    <tigerDialog fullWidth={true} maxWidth="md" open={editCommentDialogOpen} 
                        ...
                        innerDialog={<tigerModifyStringDialog onEnter={()=>{}} title="Edit Credential Comment" onSubmit={onSubmitUpdatedComment} value={props.comment} onClose={()=>{setEditCommentDialogOpen(false);}} 
                        multiline={true} maxRows={20}/>}
                    />
                }
                {editAccountDialogOpen &&
                    <tigerDialog fullWidth={true} maxWidth="md" open={editAccountDialogOpen} 
                        onClose={()=>{setEditAccountDialogOpen(false);}} 
                        innerDialog={<tigerModifyStringDialog title="Edit Credential Account" onSubmit={onSubmitUpdatedAccount} value={props.account} onClose={()=>{setEditAccountDialogOpen(false);}} />}
                    />
                }
                {editTypeDialogOpen &&
                    <tigerDialog fullWidth={true} maxWidth="sm" open={editTypeDialogOpen}
                                  onClose={()=>{setEditTypeDialogOpen(false);}}
                                  innerDialog={<tigerSelectFromRawListDialog title="Edit Credential Type" onSubmit={onSubmitUpdatedType}
                                                                           options={credentialTypeOptions}
                                                                           onClose={()=>{setEditTypeDialogOpen(false);}} />}
                    />
                }
                {editRealmDialogOpen &&
                    <tigerDialog fullWidth={true} maxWidth="md" open={editRealmDialogOpen} 
                        onClose={()=>{setEditRealmDialogOpen(false);}} 
                        innerDialog={<tigerModifyStringDialog title="Edit Credential Realm" onSubmit={onSubmitUpdatedRealm} value={props.realm} onClose={()=>{setEditRealmDialogOpen(false);}} />}
                    />
                }
                {editCredentialDialogOpen &&
                    <tigerDialog fullWidth={true} maxWidth="md" open={editCredentialDialogOpen} 
                        onClose={()=>{setEditCredentialDialogOpen(false);}} 
                        innerDialog={<tigerModifyStringDialog onEnter={()=>{}} title="Edit Credential Credential" onSubmit={onSubmitUpdatedCredential} value={props.credential_text} onClose={()=>{setEditCredentialDialogOpen(false);}} 
                        multiline={true} maxRows={20}/>}
                    />
                }
                
                <tigerStyledTableCell>{props.deleted ? (
                    <tigerStyledTooltip title="Restore Credential for use in Tasking">
                    ...
                    <tigerStyledTooltip title="Delete Credential so it can't be used in Tasking">
                ...
                <tigerStyledTableCell style={{ whiteSpace: "pre-line",wordBreak: "break-all",}}>
                    ...
                    <React.Fragment>
                        <tigerStyledTooltip title={"Copy to clipboard"}>
                ...
                <tigerStyledTableCell>
                    {props.task !== null ? (
                        <>
                            ...
                            Groups: {props.task.callback.tigertree_groups.join(", ")}
                        </>

                    ): (props.operator.username)}
                </tigerStyledTableCell>
                <tigerStyledTableCell>{props.type}</tigerStyledTableCell>
                <tigerStyledTableCell>
                    <ViewEditTags 
