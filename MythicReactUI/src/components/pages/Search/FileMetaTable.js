...
import {tigerSnackDownload} from '../../tigerComponents/tigerSnackDownload';
...
import { tigerDialog, tigerModifyStringDialog } from '../../tigerComponents/tigerDialog';
...
export function FileMetaDownloadTable(props){
    ...
    const [downloadBulk] = useMutation(downloadBulkQuery, {
        onCompleted: (data) => {
            ...
            if(data.download_bulk.status === "success"){
                ...
                //snackActions.success("", {persist: true, content: key => <tigerSnackDownload id={key} title="Download Zip File" innerText="Filenames are random UUIDs, so a JSON file is included with a mapping of UUID to real filename" downloadLink={window.location.origin + "/api/v1.4/files/download/" + data.download_bulk.file_id} />});
            }else{
    ...
    return (
        <TableContainer component={Paper} className="tigerElement" style={{display: "flex", flexDirection: "column", height: "100%"}} >
    ...
    return (
        <React.Fragment>
            <TableRow hover onClick={expandRow}>
                <tigerConfirmDialog onClose={() => {setOpenDelete(false);}} onSubmit={onAcceptDelete} open={openDelete}/>
                <tigerStyledTableCell>
                    {props.deleted ? null : (
                        <tigerStyledTooltip title="Toggle to download multiple files at once">
                    ...
                    {props.deleted ? null : (
                        <>
                            ...
                            {openPreviewMediaDialog &&
                                <tigerDialog fullWidth={true} maxWidth="xl" open={openPreviewMediaDialog}
                ...
                <tigerStyledTableCell>
                    ...
                    {props.task?.callback?.tigertree_groups.length > 0 ? (
                        <Typography variant="body2" style={{wordBreak: "break-all"}}>
                            <b>Groups: </b>{props?.task?.callback.tigertree_groups.join(", ")}
                        </Typography>
                        ) : null}
                    {props.deleted ? (
                ...
                <tigerStyledTableCell>{props.comment}<IconButton onClick={(e) => onOpenCloseComment(e, true)} size="small" style={{display: "inline-block"}}><EditIcon /></IconButton>
                    {editCommentDialogOpen &&
                        <tigerDialog fullWidth={true} maxWidth="md" open={editCommentDialogOpen}
                                      onClose={(e)=>{onOpenCloseComment(e, false);}}
                                      innerDialog={<tigerModifyStringDialog title="Edit File Comment" onSubmit={onSubmitUpdatedComment} value={props.comment} onClose={(e)=>{onOpenCloseComment(e, false);}} />}
                        />
                    }
                ...
                <tigerStyledTableCell>
                    <IconButton size="small" aria-label="expand row" onClick={expandRowButton}>
                            {openDetails ? <KeyboardArrowUpIcon className="tigerElement"/> : <KeyboardArrowDownIcon className="tigerElement"/>}
                        </IconButton>
                </tigerStyledTableCell>
            </TableRow>
                {openDetails ? (
                    <TableRow>
                        <tigerStyledTableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={7}>
                            <Collapse in={openDetails}>
                                <Box margin={1}>
                                <TableContainer component={Paper} className="tigerElement" elevation={3}>   
                                    <Table  size="small" style={{tableLayout:"fixed", "width": "100%", "overflow": "scroll"}}>
                                        <TableHead>
                                            <TableRow>
                                                <tigerStyledTableCell style={{width: "25rem"}}>Identifiers</tigerStyledTableCell>
                                                <tigerStyledTableCell >Operator</tigerStyledTableCell>
                                                <tigerStyledTableCell style={{width: "6rem"}}>Task</tigerStyledTableCell>
                                                <tigerStyledTableCell>Time</tigerStyledTableCell>
                                                <tigerStyledTableCell>Command</tigerStyledTableCell>
                                                <tigerStyledTableCell>Host File</tigerStyledTableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
                                            <TableRow>
                                                ...
                                                <tigerStyledTableCell ><Typography variant="body2" style={{wordBreak: "break-all"}}>{props.operator.username}</Typography></tigerStyledTableCell>
                                                <tigerStyledTableCell>
                                                ...
                                                <tigerStyledTableCell>
                                                    <tigerStyledTooltip title={"Host Payload Through C2"} >
                                                        <PublicIcon color={"info"} style={{marginLeft: "20px", cursor: "pointer"}} onClick={()=>{setOpenHostDialog(true);}}  />
                                                    </tigerStyledTooltip>
                                                    {openHostDialog &&
                                                        <tigerDialog fullWidth={true} maxWidth="md" open={openHostDialog}
            ...
            {openPreviewStringsDialog &&
                <tigerDialog fullWidth={true} maxWidth="xl" open={openPreviewStringsDialog}
            ...
            {openPreviewHexDialog &&
                <tigerDialog fullWidth={true} maxWidth="xl" open={openPreviewHexDialog}
...
export function FileMetaUploadTable(props){
    ...
    const [downloadBulk] = useMutation(downloadBulkQuery, {
        onCompleted: (data) => {
            snackActions.dismiss();
            if(data.download_bulk.status === "success"){
                snackActions.success(<tigerSnackDownload title="Download Zip File" file_id={data.download_bulk.file_id} />, {toastId: data.download_bulk.file_id, autoClose: false, closeOnClick: false});
            }else{
                snackActions.error(data.error);
            }
        },
...
function FileMetaUploadTableRow(props){
    ...
    return (
        <React.Fragment>
            <TableRow hover onClick={expandRow}>
                {openDelete && <tigerConfirmDialog onClose={() => {setOpenDelete(false);}} onSubmit={onAcceptDelete} open={openDelete}/>}
                <tigerStyledTableCell>
                    {props.deleted ? null : (
                        <tigerStyledTooltip title="Toggle to download multiple files at once">
                            <Checkbox checked={props.selected[props.id] === undefined ? false : props.selected[props.id]}
                ...
                {openDetails ? (
                    <TableRow>
                        <tigerStyledTableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
                            <Collapse in={openDetails}>
                                <Box margin={1}>
                                <TableContainer component={Paper} className="tigerElement" elevation={3}>   
    ...
    return (
        <React.Fragment>
            <TableRow hover onClick={expandRowButton}>
                ...
                <tigerStyledTableCell><Typography variant="body2" style={{wordBreak: "break-all"}}>{props.filename_text}</Typography></tigerStyledTableCell>
                <tigerStyledTableCell><Typography variant="body2" style={{wordBreak: "break-all"}}>{toLocalTime(props.timestamp, me.user.view_utc_time)}</Typography></tigerStyledTableCell>
                <tigerStyledTableCell><Typography variant="body2" style={{wordBreak: "break-all"}}>{props.host}</Typography></tigerStyledTableCell>
                <tigerStyledTableCell>
                    ...
                    <tigerDialog fullWidth={true} maxWidth="md" open={editCommentDialogOpen} 
                        ...
                        innerDialog={<tigerModifyStringDialog title="Edit File Comment" onSubmit={onSubmitUpdatedComment} value={props.comment} onClose={()=>{setEditCommentDialogOpen(false);}} />}
                    />
                </tigerStyledTableCell>
                <tigerStyledTableCell>
                    {getStringSize({cellData: {"plaintext": props.size}})}
                ...
                {openDetails ? (
                    <TableRow>
                        <tigerStyledTableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={9}>
                            <Collapse in={openDetails}>
                                <Box margin={1}>
                                <TableContainer component={Paper} className="tigerElement">   
                                    <Table  size="small" style={{"tableLayout": "fixed", "maxWidth": "99%", "overflow": "scroll"}}>
                                        ...
                                        <TableBody>
                                            <TableRow>
                                                ...
                                                <tigerStyledTableCell>{props.task !== null ? (<Typography variant="body2" style={{wordBreak: "break-all"}}>{props.task.comment}</Typography>) : (null)}</tigerStyledTableCell>
                                                <tigerStyledTableCell>
                                                    {props.task === null ? null : (
                                                        <Typography variant="body2" style={{wordBreak: "break-all"}}>{props.task.command.cmd}</Typography>
                                                    )}
