...
export function SocksSearchTable(props){
    ...
    return (
        <TableContainer component={Paper} className="tigerElement" style={{overflowY: "auto", flexGrow: 1, marginTop: "5px"}}>
...
function CallbackSearchTableRow(props){
    ...
    const confirmDialogText = "This does not issue any start/stop command to the agent. This only opens/closes ports that tiger controls. For rpfwd, this will not open/close that port on the remote host - you need to issue a task to your agent to do that.";
        ...
        <React.Fragment>
            <TableRow hover>
                ...
                <TableCell>{props.deleted ? (
                    <Tooltip title="Start Proxy Port on tiger Server">
                    ...
                    (<Tooltip title="Stop Proxy Port on tiger Server">
