...
${taskingDataFragment}
query responseQuery($search: String!, $offset: Int!, $fetchLimit: Int!, $status: String!) {
    task_aggregate(distinct_on: id, order_by: {id: desc}, where: {status: {_ilike: $status}, callback: {tigertree_groups_string: {_ilike: $search}}}) {
      aggregate {
        count(columns: id)
      }
    }
    task(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {status: {_ilike: $status}, callback: {tigertree_groups_string: {_ilike: $search}}}) {
      ...taskData
    }
  }
`;
...
export function SearchTabTasksLabel(props){
    return (
        <tigerSearchTabLabel label={"Tasks"} iconComponent={<AssignmentIcon />} {...props}/>
    )
}
...
const SearchTabTasksSearchPanel = (props) => {
    ...
    return (
        <Grid container spacing={2} style={{paddingTop: "10px", paddingLeft: "10px", maxWidth: "100%"}}>
            ...
            <Grid item xs={4}>
                <tigerTextField placeholder="Filter Task Status..." value={filterTaskStatus}
