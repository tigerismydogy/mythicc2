		...
		*
		...
		if user.Deleted {
			...
			if scriptingVersion == "" {
				...
				go rabbitmq.SendAllOperationsMessage(fmt.Sprintf("%s connected via tiger Scripting (v%s) from %s", username, scriptingVersion, fromIP), 0, "", database.MESSAGE_LEVEL_DEBUG)
			}
