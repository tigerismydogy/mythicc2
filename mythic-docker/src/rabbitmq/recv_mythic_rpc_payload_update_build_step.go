...
type tigerRPCPayloadUpdateBuildStepMessage struct {
...
type tigerRPCPayloadUpdateBuildStepMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_PAYLOAD_UPDATE_BUILD_STEP,   // swap out with queue in rabbitmq.constants.go file
		RoutingKey: tiger_RPC_PAYLOAD_UPDATE_BUILD_STEP,   // swap out with routing key in rabbitmq.constants.go file
		Handler:    processtigerRPCPayloadUpdateBuildStep, // points to function that takes in amqp.Delivery and returns interface{}
	})
}

// tiger_RPC_OBJECT_ACTION - Say what the function does
func tigerRPCPayloadUpdateBuildStep(input tigerRPCPayloadUpdateBuildStepMessage) tigerRPCPayloadUpdateBuildStepMessageResponse {
	response := tigerRPCPayloadUpdateBuildStepMessageResponse{
		Success: false,
	}
...
func processtigerRPCPayloadUpdateBuildStep(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCPayloadUpdateBuildStepMessage{}
	responseMsg := tigerRPCPayloadUpdateBuildStepMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCPayloadUpdateBuildStep(incomingMessage)
	}
	return responseMsg
}
