...
type tigerRPCCallbackCreateMessage struct {
...
type tigerRPCCallbackCreateMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_CALLBACK_CREATE,
		RoutingKey: tiger_RPC_CALLBACK_CREATE,
		Handler:    processtigerRPCCallbackCreate,
	})
}

// Endpoint: tiger_RPC_CALLBACK_CREATE
func tigerRPCCallbackCreate(input tigerRPCCallbackCreateMessage) tigerRPCCallbackCreateMessageResponse {
	response := tigerRPCCallbackCreateMessageResponse{
		Success: false,
	}
	...
	if err != nil {
		logging.LogError(err, "Failed to find payload when calling tigerRPCCallbackCreate")
	...
	statement, err := transaction.PrepareNamed(`
			...
			RETURNING *`)
	if err != nil {
		logging.LogError(err, "Failed to create callback in database in tigerRPCCallbackCreate")
		response.Error = err.Error()
		return response
	}
	...
	if err != nil {
		logging.LogError(err, "Failed to get associated commands in payload when creating new callback in tigerRPCCallbackCreate")
		response.Error = err.Error()
		return response
	}
	...
	for _, pc2p := range payloadC2Profiles {
		if err := database.DB.Select(&payloadC2ProfileParameterInstances, `SELECT
			...
			logging.LogError(err, "Failed to get c2 profile parameter instances from payload in tigerRPCCallbackCreate")
			response.Error = err.Error()
			return response
		...
		if !pc2p.C2profile.IsP2p && pc2p.C2profile.Name == input.C2ProfileName {
			...
			if _, err := database.DB.NamedExec(`INSERT INTO callbackgraphedge
				...
				logging.LogError(err, "Failed to create new callbackgraphedge for egress c2 profile in tigerRPCCallbackCreate")
				response.Error = err.Error()
				return response
			} else {
...
func processtigerRPCCallbackCreate(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCCallbackCreateMessage{}
	responseMsg := tigerRPCCallbackCreateMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCCallbackCreate(incomingMessage)
	}
	return responseMsg
}
