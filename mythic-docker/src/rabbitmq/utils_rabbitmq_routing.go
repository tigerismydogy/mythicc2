	...
	if r.conn != nil && !r.conn.IsClosed() {
		...
		for {
			...
			conn, err := amqp.DialConfig(fmt.Sprintf("amqp://%s:%s@%s:%d/%s",
				utils.tigerConfig.RabbitmqUser,
				utils.tigerConfig.RabbitmqPassword,
				utils.tigerConfig.RabbitmqHost,
				utils.tigerConfig.RabbitmqPort,
				utils.tigerConfig.RabbitmqVHost),
				amqp.Config{
...
func (r *rabbitMQConnection) SendMessage(exchange string, queue string, correlationId string, body []byte, ignoreErrormessage bool) error {
	...
	if err != nil && exchange == tiger_TOPIC_EXCHANGE {
		return nil
	} else {
		return err
	}
...
func (r *rabbitMQConnection) ReceiveFromtigerDirectExchange(exchange string, queue string, routingKey string, handler QueueHandler, exclusiveQueue bool) {
...
func (r *rabbitMQConnection) GetNumberOfConsumersDirectChannels(exchange string, kind string, queue string) (uint, error) {
	...
	if err != nil {
		logging.LogError(err, "Failed to declare exchange", "exchange", tiger_TOPIC_EXCHANGE, "exchange_type", "topic", "retry_wait_time", RETRY_CONNECT_DELAY)
		return 0, err
...
func (r *rabbitMQConnection) ReceiveFromtigerDirectTopicExchange(exchange string, queue string, routingKey string, handler QueueHandler, exclusiveQueue bool) {
	// exchange is a direct exchange
	// queue is where the messages get sent to (local name)
	// routingKey is the specific direct topic we're interested in for the exchange
	// handler processes the messages we get on our queue
...
func (r *rabbitMQConnection) CheckPayloadTypeContainerExists(name string) (bool, error) {
	exclusiveQueue := true
	return r.CheckConsumerExists(tiger_EXCHANGE, GetPtBuildRoutingKey(name), !exclusiveQueue)
}
func (r *rabbitMQConnection) CheckC2ProfileContainerExists(name string) (bool, error) {
	exclusiveQueue := true
	return r.CheckConsumerExists(tiger_EXCHANGE, GetC2RPCStartServerRoutingKey(name), exclusiveQueue)
}
func getMeaningfulRabbitmqError(ret amqp.Return) string {
	switch ret.ReplyCode {
	case 312:
		return fmt.Sprintf("#No RabbitMQ Route for %s. Is the container online (./tiger-cli status)?\n#If the container is online, there might be an issue within the container processing the request (./tiger-cli logs [container name]). ", ret.RoutingKey)
	default:
