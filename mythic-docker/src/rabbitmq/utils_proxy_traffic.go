...
type callbackPortsInUse struct {
	...
	proxyFromAgentMessageChannel chan ProxyFromAgentMessageFortiger
	bytesReceivedFromAgentChan   chan bytesReceivedFromAgentMessage
	bytesSentToAgentChan         chan bytesSentToAgentMessage
}
...
type ProxyFromAgentMessageFortiger struct {
	CallbackID          int
...
func (c *callbackPortsInUse) Initialize() {
	...
	c.proxyFromAgentMessageChannel = make(chan ProxyFromAgentMessageFortiger, 2000)
...
func (c *callbackPortsInUse) GetNextAvailableLocalPort() uint32 {
	exposedPorts := utils.tigerConfig.ServerDynamicPorts
...
func (c *callbackPortsInUse) IsPortInUse(port uint32) bool {
	for i, _ := range c.ports {
		...
		case CALLBACK_PORT_TYPE_RPORTFWD:
			// rpfwd doesn't bind a port on tiger, so the ports don't count here
			continue
		case CALLBACK_PORT_TYPE_SOCKS:
			fallthrough
		case CALLBACK_PORT_TYPE_INTERACTIVE:
...
func (c *callbackPortsInUse) SendDataToCallbackIdPortType(callbackId int, portType CallbackPortType, messages []proxyFromAgentMessage) {
	c.proxyFromAgentMessageChannel <- ProxyFromAgentMessageFortiger{
...
func isPortExposedThroughDocker(portToCheck int) bool {
	for _, port := range utils.tigerConfig.ServerDynamicPorts {
...
func (p *callbackPortUsage) GetProxyData() []proxyToAgentMessage {
	...
	for i := 0; i < len(messagesToSendToAgent); i++ {
		...
		case messagesToSendToAgent[i] = <-p.messagesToAgent:
			//logging.LogDebug("Agent picking up msg from tiger", "serverID", messagesToSendToAgent[i].ServerID, "exit", messagesToSendToAgent[i].IsExit)
		default:
			//logging.LogDebug("returning set of messages to agent from tiger", "msgs", messagesToSendToAgent)
...
func (p *callbackPortUsage) GetInteractiveData() []agentMessagePostResponseInteractive {
	...
	for i := 0; i < len(messagesToSendToAgent); i++ {
		...
		case messagesToSendToAgent[i] = <-p.interactiveMessagesToAgent:
			//logging.LogDebug("Got message from tiger to agent", "TaskID", messagesToSendToAgent[i].TaskUUID)
		default:
			//logging.LogDebug("returning set of messages to agent from tiger", "msgs", messagesToSendToAgent)
			// this is in case we run out of messages for some reason
			return messagesToSendToAgent
		...
		case newMsg := <-p.messagesFromAgent:
			...
			case CALLBACK_PORT_TYPE_SOCKS:
				...
				if _, ok := connectionMap[newMsg.ServerID]; ok {
					//logging.LogInfo("got msg from agent for server tiger still thinks is alive", "serverID", newMsg.ServerID, "exit", newMsg.IsExit)
					select {
...
func (p *callbackPortUsage) handleSocksConnections() {
	for {
		...
		go func(conn net.Conn) {
			...
			go func(conn net.Conn) {
				// function for reading from agents to send to tiger's connections
				for {
					select {
			...
			go func(conn net.Conn) {
				// function for reading from tiger's connections to send to agents
				for {
					buf := make([]byte, 4096)
					...
					if err != nil {
						if err != io.EOF {
							...
							//logging.LogInfo("Got normal EOF from connection, exiting on tiger side", "serverID", newConnection.ServerID)
						}
						if length > 0 {
