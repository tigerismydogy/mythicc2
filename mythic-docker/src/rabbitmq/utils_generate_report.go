...
func GenerateReport(reportConfig GenerateReportMessage) {
	...
	if reportConfig.OutputFormat == "json" {
		...
		tigerSvgContents, _ := os.ReadFile(filepath.Join(".", "static", "red_blue_login.png"))
		report := XMLDiv{
			Style: ``,
			Body: []interface{}{
				XMLDiv{
					...
					Body: []interface{}{
						XMLPng{
							...
							Contents: fmt.Sprintf("data:image/png;base64, %s", base64.StdEncoding.EncodeToString(tigerSvgContents)),
						},
						XMLDiv{
