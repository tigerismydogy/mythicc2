...
type tigerRPCFileUpdateMessage struct {
...
type tigerRPCFileUpdateMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_FILE_UPDATE,     // swap out with queue in rabbitmq.constants.go file
		RoutingKey: tiger_RPC_FILE_UPDATE,     // swap out with routing key in rabbitmq.constants.go file
		Handler:    processtigerRPCFileUpdate, // points to function that takes in amqp.Delivery and returns interface{}
	})
}

// tiger_RPC_OBJECT_ACTION - Say what the function does
func tigerRPCFileUpdate(input tigerRPCFileUpdateMessage) tigerRPCFileUpdateMessageResponse {
	response := tigerRPCFileUpdateMessageResponse{
		Success: false,
	}
...
func processtigerRPCFileUpdate(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCFileUpdateMessage{}
	responseMsg := tigerRPCFileUpdateMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCFileUpdate(incomingMessage)
	}
	return responseMsg
}
