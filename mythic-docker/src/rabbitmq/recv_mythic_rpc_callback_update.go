...
type tigerRPCCallbackUpdateMessage struct {
...
type tigerRPCCallbackUpdateMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_CALLBACK_UPDATE,
		RoutingKey: tiger_RPC_CALLBACK_UPDATE,
		Handler:    processtigerRPCCallbackUpdate,
	})
}

// Endpoint: tiger_RPC_CALLBACK_UPDATE
func tigerRPCCallbackUpdate(input tigerRPCCallbackUpdateMessage) tigerRPCCallbackUpdateMessageResponse {
	response := tigerRPCCallbackUpdateMessageResponse{
		Success: false,
	}
...
func processtigerRPCCallbackUpdate(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCCallbackUpdateMessage{}
	responseMsg := tigerRPCCallbackUpdateMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCCallbackUpdate(incomingMessage)
	}
	return responseMsg
}
