...
func checkContainerStatus() {
	...
	// http://rabbitmq_user:rabbitmq_password@rabbitmq_host:15672/rabbitmq/api/queues/tiger_vhost
	rabbitmqReqURL := fmt.Sprintf("http://%s:%s@%s:15672/api/queues/tiger_vhost?use_regex=true&page=1&page_size=500&name=%s",
		utils.tigerConfig.RabbitmqUser, utils.tigerConfig.RabbitmqPassword, utils.tigerConfig.RabbitmqHost,
