...
type tigerRPCPayloadCreateFromUUIDMessage struct {
...
type tigerRPCPayloadCreateFromUUIDMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_PAYLOAD_CREATE_FROM_UUID,   // swap out with queue in rabbitmq.constants.go file
		RoutingKey: tiger_RPC_PAYLOAD_CREATE_FROM_UUID,   // swap out with routing key in rabbitmq.constants.go file
		Handler:    processtigerRPCPayloadCreateFromUUID, // points to function that takes in amqp.Delivery and returns interface{}
	})
}

// tiger_RPC_OBJECT_ACTION - Say what the function does
func tigerRPCPayloadCreateFromUUID(input tigerRPCPayloadCreateFromUUIDMessage) tigerRPCPayloadCreateFromUUIDMessageResponse {
	response := tigerRPCPayloadCreateFromUUIDMessageResponse{
		Success: false,
	}
...
func processtigerRPCPayloadCreateFromUUID(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCPayloadCreateFromUUIDMessage{}
	responseMsg := tigerRPCPayloadCreateFromUUIDMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCPayloadCreateFromUUID(incomingMessage)
	}
	return responseMsg
}
		...
		for rows.Next() {
			if err = rows.StructScan(&databaseBuildParameter); err != nil {
				...
				if databaseBuildParameter.IsCryptoType {
					if databasePayload.Payloadtype.TranslationContainerID.Valid && !databasePayload.Payloadtype.tigerEncrypts {
						...
						if cryptoKeys, err := tigerCrypto.GenerateKeysForPayload(finalBuildParameters[databaseBuildParameter.Name]); err != nil {
				...
				if err = rows.StructScan(&databaseC2ProfileParameter); err != nil {
					...
					if databaseC2ProfileParameter.IsCryptoType {
						if databasePayload.Payloadtype.TranslationContainerID.Valid && !databasePayload.Payloadtype.tigerEncrypts {
							...
							if cryptoKeys, err := tigerCrypto.GenerateKeysForPayload(paramStringVal); err != nil {
								logging.LogError(err, "Failed to generate crypto keys for payload")
								return nil, err
							} else {
								if cryptoKeys.EncKey != nil {
