...
func AgentMessageWebhook(c *gin.Context) {
	...
	c2Header := c.GetHeader("tiger")
	if c2Header == "" {
		logging.LogError(err, "Failed to get 'tiger' header")
		errorMessage := "Error! Failed to find tiger header. Check the following details for more information about the request:\nConnection to: "
		errorMessage += fmt.Sprintf("%s via HTTP %s\nFrom: %s\n", requestUrl, c.Request.Method, requestIp)
		errorMessage += "Did this come from a tiger C2 Profile? If so, make sure it's adding the `tiger` header with the name of the C2 profile"
		go rabbitmq.SendAllOperationsMessage(errorMessage, 0, "agent_message_missing_tiger_header", database.MESSAGE_LEVEL_WARNING)
		c.Status(http.StatusNotFound)
		return
	}
...
func AgentMessageGetWebhook(c *gin.Context) {
	...
	c2Header := c.GetHeader("tiger")
	if c2Header == "" {
		logging.LogError(nil, "Failed to get 'tiger' header")
		errorMessage := "Error! Failed to find tiger header. Check the following details for more information about the request:\nConnection to: "
		errorMessage += fmt.Sprintf("%s via HTTP %s\nFrom: %s\n", requestUrl, c.Request.Method, requestIp)
		errorMessage += "Did this come from a tiger C2 Profile? If so, make sure it's adding the `tiger` header with the name of the C2 profile"
		go rabbitmq.SendAllOperationsMessage(errorMessage, 0, "agent_message_missing_tiger_header", database.MESSAGE_LEVEL_WARNING)
