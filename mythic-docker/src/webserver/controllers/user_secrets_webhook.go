...
func UpdateSecretsWebhook(c *gin.Context) {
	...
	tigerJsonText := rabbitmq.GettigerJSONTextFromStruct(input.Input.Secrets)
	_, err = database.DB.Exec(`UPDATE operator SET secrets=$1 WHERE id=$2`, tigerJsonText, userID)
	if err != nil {
