...
func (d *DockerComposeManager) SaveImages(services []string, outputPath string) error {
	...
	if len(savedContainers) == 0 {
		...
		currenttigerServices, err := d.GetCurrenttigerServiceNames()
		if err != nil {
			return errors.New(fmt.Sprintf("[-] Failed to get tiger service list: %v\n", err))
		}
		savedContainers = append([]string{}, diskAgents...)
		savedContainers = append(savedContainers, currenttigerServices...)

	}
	savedImagePath = filepath.Join(utils.GetCwdFromExe(), "saved_images", "tiger_save.tar")
...
func (d *DockerComposeManager) LoadImages(outputPath string) error {
	savedImagePath := filepath.Join(utils.GetCwdFromExe(), outputPath, "tiger_save.tar")
	cli, err := client.NewClientWithOpts(client.FromEnv, client.WithAPIVersionNegotiation())
	if err != nil {
		return errors.New(fmt.Sprintf("[-] Failed to connect to Docker: %v\n", err))
	}
...
func (d *DockerComposeManager) CheckRequiredManagerVersion() bool {
	...
	log.Printf("[-] Docker version is too old, %s, for tiger. Please update\n", outputString)
	return false
...
func (d *DockerComposeManager) StopServices(services []string, deleteImages bool) error {
	...
	if len(services) == 0 {
		services = append(dockerComposeContainers, currenttigerServices...)
	}
	/*
		if utils.StringInSlice("tiger_react", services) {
			if tigerEnv.GetBool("tiger_react_debug") {
				// only need to remove the container if we're switching between debug and regular
				if err = d.runDockerCompose(append([]string{"rm", "-s", "-v", "-f"}, "tiger_react")); err != nil {
					fmt.Printf("[-] Failed to remove tiger_react\n")
					return err
				}
			}
		}
...
func (d *DockerComposeManager) BuildUI() error {
	_, err := d.runDocker([]string{"exec", "tiger_react", "/bin/sh", "-c", "npm run react-build"})
	if err != nil {
		log.Printf("[-] Failed to build new UI from tigerReactUI: %v\n", err)
	}
	return err
}
...
func (d *DockerComposeManager) TestPorts(services []string) {
	// go through the different services in tigerEnv and check to make sure their ports aren't already used by trying to open them
	//tiger_SERVER_HOST:tiger_SERVER_PORT
	...
	for key, val := range portChecks {
		...
		if utils.StringInSlice(val[1], services) {
			if tigerEnv.GetString(key) == val[1] || tigerEnv.GetString(key) == "127.0.0.1" {
				addServices = append(addServices, val[1])
				p, err := net.Listen("tcp", ":"+strconv.Itoa(tigerEnv.GetInt(val[0])))
				if err != nil {
					log.Fatalf("[-] Port %d, from variable %s, appears to already be in use: %v\n", tigerEnv.GetInt(val[0]), key, err)
				}
				err = p.Close()
				if err != nil {
...
func (d *DockerComposeManager) PrintConnectionInfo() {
	...
	fmt.Fprintln(w, "tiger SERVICE\tWEB ADDRESS\tBOUND LOCALLY")
	if tigerEnv.GetString("NGINX_HOST") == "tiger_nginx" {
		if tigerEnv.GetBool("NGINX_USE_SSL") {
			fmt.Fprintln(w, "Nginx (tiger Web UI)\thttps://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("NGINX_PORT"))+"\t", tigerEnv.GetBool("nginx_bind_localhost_only"))
		} else {
			fmt.Fprintln(w, "Nginx (tiger Web UI)\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("NGINX_PORT"))+"\t", tigerEnv.GetBool("nginx_bind_localhost_only"))
		}
	} else {
		if tigerEnv.GetBool("NGINX_USE_SSL") {
			fmt.Fprintln(w, "Nginx (tiger Web UI)\thttps://"+tigerEnv.GetString("NGINX_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("NGINX_PORT"))+"\t", tigerEnv.GetBool("nginx_bind_localhost_only"))
		} else {
			fmt.Fprintln(w, "Nginx (tiger Web UI)\thttp://"+tigerEnv.GetString("NGINX_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("NGINX_PORT"))+"\t", tigerEnv.GetBool("nginx_bind_localhost_only"))
		}
	}
	if tigerEnv.GetString("tiger_SERVER_HOST") == "tiger_server" {
		fmt.Fprintln(w, "tiger Backend Server\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("tiger_SERVER_PORT"))+"\t", tigerEnv.GetBool("tiger_server_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "tiger Backend Server\thttp://"+tigerEnv.GetString("tiger_SERVER_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("tiger_SERVER_PORT"))+"\t", tigerEnv.GetBool("tiger_server_bind_localhost_only"))
	}
	if tigerEnv.GetString("HASURA_HOST") == "tiger_graphql" {
		fmt.Fprintln(w, "Hasura GraphQL Console\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("HASURA_PORT"))+"\t", tigerEnv.GetBool("hasura_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "Hasura GraphQL Console\thttp://"+tigerEnv.GetString("HASURA_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("HASURA_PORT"))+"\t", tigerEnv.GetBool("hasura_bind_localhost_only"))
	}
	if tigerEnv.GetString("JUPYTER_HOST") == "tiger_jupyter" {
		fmt.Fprintln(w, "Jupyter Console\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("JUPYTER_PORT"))+"\t", tigerEnv.GetBool("jupyter_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "Jupyter Console\thttp://"+tigerEnv.GetString("JUPYTER_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("JUPYTER_PORT"))+"\t", tigerEnv.GetBool("jupyter_bind_localhost_only"))
	}
	if tigerEnv.GetString("DOCUMENTATION_HOST") == "tiger_documentation" {
		fmt.Fprintln(w, "Internal Documentation\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("DOCUMENTATION_PORT"))+"\t", tigerEnv.GetBool("documentation_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "Internal Documentation\thttp://"+tigerEnv.GetString("DOCUMENTATION_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("DOCUMENTATION_PORT"))+"\t", tigerEnv.GetBool("documentation_bind_localhost_only"))
	}
	fmt.Fprintln(w, "\t\t\t\t")
	fmt.Fprintln(w, "ADDITIONAL SERVICES\tADDRESS\tBOUND LOCALLY")
	if tigerEnv.GetString("POSTGRES_HOST") == "tiger_postgres" {
		fmt.Fprintln(w, "Postgres Database\tpostgresql://tiger_user:password@127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("POSTGRES_PORT"))+"/tiger_db\t", tigerEnv.GetBool("postgres_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "Postgres Database\tpostgresql://tiger_user:password@"+tigerEnv.GetString("POSTGRES_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("POSTGRES_PORT"))+"/tiger_db\t", tigerEnv.GetBool("postgres_bind_localhost_only"))
	}
	if tigerEnv.GetString("tiger_REACT_HOST") == "tiger_react" {
		fmt.Fprintln(w, "React Server\thttp://127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("tiger_REACT_PORT"))+"/new\t", tigerEnv.GetBool("tiger_react_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "React Server\thttp://"+tigerEnv.GetString("tiger_REACT_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("tiger_REACT_PORT"))+"/new\t", tigerEnv.GetBool("tiger_react_bind_localhost_only"))
	}
	if tigerEnv.GetString("RABBITMQ_HOST") == "tiger_rabbitmq" {
		fmt.Fprintln(w, "RabbitMQ\tamqp://"+tigerEnv.GetString("RABBITMQ_USER")+":password@127.0.0.1:"+strconv.Itoa(tigerEnv.GetInt("RABBITMQ_PORT"))+"\t", tigerEnv.GetBool("rabbitmq_bind_localhost_only"))
	} else {
		fmt.Fprintln(w, "RabbitMQ\tamqp://"+tigerEnv.GetString("RABBITMQ_USER")+":password@"+tigerEnv.GetString("RABBITMQ_HOST")+":"+strconv.Itoa(tigerEnv.GetInt("RABBITMQ_PORT"))+"\t", tigerEnv.GetBool("rabbitmq_bind_localhost_only"))
	}
	fmt.Fprintln(w, "\t\t\t\t")
	w.Flush()
}
...
func (d *DockerComposeManager) Status(verbose bool) {
	...
	for _, c := range containers {
		...
		if !foundMountInfo {
			if c.Labels["name"] == "tiger_graphql" {
				info += "N/A"
			} else {
				info += "local"
			}
		...
		info += "\t"
		if utils.StringInSlice(c.Labels["name"], config.tigerPossibleServices) {
			info = info + portString
			tigerLocalServices = append(tigerLocalServices, info)
		} else {
	...
	fmt.Fprintln(w, "tiger Main Services")
	fmt.Fprintln(w, "CONTAINER NAME\tSTATE\tSTATUS\tMOUNT\tPORTS")
	for _, line := range tigerLocalServices {
		fmt.Fprintln(w, line)
	}
	fmt.Fprintln(w, "\t\t\t\t\t")
	w.Flush()
	...
	if len(elementsInCompose) > 0 && verbose {
		fmt.Fprintln(w, "Docker Compose services not running, start with: ./tiger-cli start [name]")
		fmt.Fprintln(w, "NAME\t")
	...
	if len(elementsOnDisk) > 0 && verbose {
		fmt.Fprintln(w, "Extra Services, add to docker compose with: ./tiger-cli add [name]")
		fmt.Fprintln(w, "NAME\t")
...
func (d *DockerComposeManager) ResetDatabase(useVolume bool) {
	if !useVolume {
		...
		_ = d.RemoveContainers([]string{"tiger_postgres"})
		err := d.RemoveVolume("tiger_postgres_volume")
		if err != nil {
...
func (d *DockerComposeManager) BackupDatabase(backupPath string, useVolume bool) error {
	if !useVolume {
		...
		defer cli.Close()
		execID, err := cli.ContainerExecCreate(ctx, "tiger_postgres", types.ExecConfig{
			AttachStderr: true,
			AttachStdout: true,
			AttachStdin:  true,
		...
		tarFileName := fmt.Sprintf("%s-tiger_postgres.tar", todayString)
		defer session.Close()
		dumpCommand := fmt.Sprintf("PGPASSWORD=%s pg_dump -n public --format=tar -U tiger_user -f /var/lib/postgresql/data/%s tiger_db\n",
			config.GettigerEnv().GetString("postgres_password"), tarFileName)
		...
		err = d.CopyFromVolume("tiger_postgres_volume", tarFileName, backupPath)
		if err != nil {
			return err
		}
...
func (d *DockerComposeManager) RestoreDatabase(backupPath string, useVolume bool) error {
	...
	if !useVolume {
		...
		err := d.CopyIntoVolume(backupPath, "dump.tar", "tiger_postgres_volume")
		if err != nil {
			return err
		}
		log.Printf("[+] Successfully copied database files")
		...
		defer session.Close()
		dumpCommand := fmt.Sprintf("PGPASSWORD=%s pg_restore -U tiger_user -n public --clean --if-exists -d tiger_db /var/lib/postgresql/data/dump.tar\n",
			config.GettigerEnv().GetString("postgres_password"))
		_, err = session.Conn.Write([]byte(dumpCommand))
		if err != nil {
			log.Fatalf("[!] Failed to write to exec bash: %v", err)
		} else {
...
func (d *DockerComposeManager) BackupFiles(backupPath string, useVolume bool) error {
	if !useVolume {
		workingPath := utils.GetCwdFromExe()
		log.Printf("[*] Staring to copy, this might take a minute...")
		err := utils.CopyDir(filepath.Join(workingPath, "tiger-docker", "src", "files"), backupPath)
		if err != nil {
			log.Printf("[-] Failed to copy tiger's uploads/downloads\n%v\n", err)
			return err
		} else {
			log.Printf("[+] Successfully copied tiger's uploads/downloads from disk\n")
			return nil
		}
	} else {
		err := d.CopyFromVolume("tiger_server_volume", "", backupPath)
		if err != nil {
			return err
		}
		log.Printf("[+] Successfully copied tiger's uploads/downloads from volume")
		return nil
	}
}
func (d *DockerComposeManager) RestoreFiles(backupPath string, useVolume bool) error {
	...
	if !useVolume {
		workingPath := utils.GetCwdFromExe()
		log.Printf("[*] Staring to copy, this might take a minute...")
		err := utils.CopyDir(backupPath, filepath.Join(workingPath, "tiger-docker", "src", "files"))
		if err != nil {
			log.Printf("[-] Failed to copy tiger's uploads/downloads\n%v\n", err)
			return err
		} else {
			log.Printf("[+] Successfully copied tiger's uploads/downloads\n")
			return nil
		}
	} else {
		err := d.CopyIntoVolume(backupPath, "/", "tiger_server_volume")
		if err != nil {
			return err
		}
		log.Printf("[+] Successfully copied tiger's uploads/downloads")
...
func (d *DockerComposeManager) gettigerEnvList() []string {
	env := config.GettigerEnv().AllSettings()
	var envList []string
	for key := range env {
		val := config.GettigerEnv().GetString(key)
		if val != "" {
...
// GetCurrenttigerServiceNames from reading in the docker-compose file, not necessarily what should be there or what's running
func (d *DockerComposeManager) GetCurrenttigerServiceNames() ([]string, error) {
	groupNameConfig := viper.New()
	groupNameConfig.SetConfigName("docker-compose")
	groupNameConfig.SetConfigType("yaml")
	groupNameConfig.AddConfigPath(utils.GetCwdFromExe())
