...
func InstallFolder(installPath string, overWrite bool) error {
	...
	for key, val := range latestVersion {
		...
		if !config.GettigerEnv().InConfig(fmt.Sprintf("%s_use_volume", key)) {
			config.SetNewConfigStrings(fmt.Sprintf("%s_use_volume", key), "true")
		}
		if !config.GettigerEnv().InConfig(fmt.Sprintf("%s_use_build_context", key)) {
		...
		if err != nil {
			...
			for _, f := range files {
				if f.IsDir() {
					...
					if !config.GettigerEnv().GetBool("documentation_use_volume") {
						...
						err = moveFileToVolume("tiger_documentation_volume",
	...
	if !installConfig.GetBool("exclude_documentation_c2") {
		...
		if err != nil {
			...
			for _, f := range files {
				if f.IsDir() {
					log.Printf("[*] Processing Documentation for %s\n", f.Name())
					if !config.GettigerEnv().GetBool("tiger_documentation_use_volume") {
	...
	if manager.GetManager().IsServiceRunning("tiger_documentation") {
		fmt.Printf("[*] Restarting tiger_documentation container to pull in changes\n")
		ServiceStop([]string{"tiger_documentation"})
		ServiceStart([]string{"tiger_documentation"})
	}
	return nil
}
...
func installServiceByName(service string) error {
	// just have a service name, check tigerAgents, tigerC2Profiles, or error
	agentURL := fmt.Sprintf("https://github.com/tigerAgents/%s", service)
	c2URL := fmt.Sprintf("https://github.com/tigerC2Profiles/%s", service)
...
func UninstallService(services []string) {
	...
	for _, service := range services {
		if utils.StringInSlice(strings.ToLower(service), config.tigerPossibleServices) {
			log.Fatalf("[-] Trying to uninstall tiger services not allowed\nIf you need to remove it locally send the corresponding environment host name to an IP address or hostname")
		}
		found := false
		...
		if found {
			...
			if manager.GetManager().IsServiceRunning("tiger_documentation") {
				log.Printf("[*] Restarting tiger_documentation container to pull in changes\n")
				ServiceStop([]string{"tiger_documentation"})
				ServiceStart([]string{"tiger_documentation"})
			}
			return
...
func InstalltigerSyncFolder(installPath string) error {
	service := "tiger_sync"
	if manager.GetManager().IsServiceRunning(service) {
		err := manager.GetManager().StopServices([]string{service}, config.GettigerEnv().GetBool("REBUILD_ON_START"))
		if err != nil {
	...
	if err != nil {
		log.Printf("[-] Failed to create %s directory to install tiger_sync: %v\n", service, err)
		return err
	}
	AddtigerService(service, true)
	log.Printf("[+] Successfully installed tiger_sync!\n")
	if manager.GetManager().IsServiceRunning("tiger_server") {
		log.Printf("[*] Starting tiger_sync")
		err = ServiceStart([]string{strings.ToLower(service)})
		if err != nil {
			log.Printf("[-] Failed to start tiger_sync: %v\n", err)
		}
	}
	return nil
}
func InstalltigerSync(url string, branch string) error {
	// make our temp directory to clone into
	workingPath := utils.GetCwdFromExe()
	log.Printf("[*] Creating temporary directory\n")
	if utils.DirExists(filepath.Join(workingPath, "tmp")) {
	...
	return InstalltigerSyncFolder(filepath.Join(workingPath, "tmp"))

}
func UninstalltigerSync() {
	service := "tiger_sync"
	if manager.GetManager().IsServiceRunning(service) {
