...
func generateCerts() error {
	...
	certPath := filepath.Join(utils.GetCwdFromExe(), "nginx-docker", "ssl", "tiger-cert.crt")
	keyPath := filepath.Join(utils.GetCwdFromExe(), "nginx-docker", "ssl", "tiger-ssl.key")
...
func updateNginxBlockLists() {
	ipString := config.GettigerEnv().GetString("allowed_ip_blocks")
	...
	if !config.GettigerEnv().GetBool("nginx_bind_use_volume") {
		...
		err := moveStringToVolume("tiger_nginx_volume_config", ".", "blockips.conf", outputString)
		if err != nil {
			log.Fatalf("[-] Failed to write out block list file: %v\n", err)
		}
	}
