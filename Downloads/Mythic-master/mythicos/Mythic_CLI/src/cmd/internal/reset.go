...
func DatabaseReset(force bool) {
	if force {
		log.Printf("[*] Stopping tiger\n")
		manager.GetManager().StopServices([]string{}, config.GettigerEnv().GetBool("REBUILD_ON_START"))
		manager.GetManager().ResetDatabase(config.GettigerEnv().GetBool("postgres_use_volume"))
...
func DatabaseBackup(backupPath string) {
	...
	if confirm {
		err := manager.GetManager().BackupDatabase(backupPath, config.GettigerEnv().GetBool("postgres_use_volume"))
		if err != nil {
...
func DatabaseRestore(backupPath string) {
	...
	if confirm {
		...
		if confirm {
			err := manager.GetManager().RestoreDatabase(backupPath, config.GettigerEnv().GetBool("postgres_use_volume"))
			if err != nil {
				log.Fatalf("[-] Failed to restore database: %v\n", err)
			} else {
				manager.GetManager().StopServices([]string{"tiger_postgres"}, false)
				manager.GetManager().StartServices([]string{"tiger_postgres"}, config.GettigerEnv().GetBool("REBUILD_ON_START"))
			}
		}
	}
}
func FilesBackup(backupPath string) {
	...
	if confirm {
		err := manager.GetManager().BackupFiles(backupPath, config.GettigerEnv().GetBool("tiger_server_use_volume"))
		if err != nil {
			log.Fatalf("[-] Failed to backup tiger's files: %v\n", err)
		}
	}
}
func FilesRestore(backupPath string) {
	...
	if confirm {
		...
		if confirm {
			err := manager.GetManager().RestoreFiles(backupPath, config.GettigerEnv().GetBool("tiger_server_use_volume"))
			if err != nil {
				log.Fatalf("[-] Failed to restore files: %v\n", err)
			} else {
				manager.GetManager().StopServices([]string{"tiger_server"}, false)
				manager.GetManager().StartServices([]string{"tiger_server"}, config.GettigerEnv().GetBool("REBUILD_ON_START"))
			}
		}
	}
}
