...
type CLIManager interface {
	...
	// GetServiceConfiguration gets the current configuration for a tiger or 3rd party service
	GetServiceConfiguration(string) (map[string]interface{}, error)
	// SetServiceConfiguration sets the specified configuration for a tiger or specified 3rd party service
	...
	// GetAllExistingNontigerServiceNames reads current configuration and returns all non-tiger services
	GetAllInstalled3rdPartyServiceNames() ([]string, error)
	// GetCurrenttigerServiceNames reads current configuration and returns all tiger services
	GetCurrenttigerServiceNames() ([]string, error)
	...
	// BuildUI a new instance of the tiger React UI and save it in the tiger-react-docker folder
	BuildUI() error
	...
	// BackupFiles saves the files associated with tiger's uploads/downloads to the specified path
	BackupFiles(backupPath string, useVolume bool) error
	// RestoreFiles restores a saved copy of tiger's uploads/downloads from the specified path
	RestoreFiles(backupPath string, useVolume bool) error
	// PrintVolumeInformation prints out all the volumes in use by tiger
...
func Initialize() {
	envManager := config.GettigerEnv().GetString("global_manager")
	switch envManager {
	case "docker":
