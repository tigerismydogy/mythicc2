...
- Added a -f (--force) flag for the `sudo ./tiger-cli database reset` command to not prompt for confirmation to help with automation
...
- Updated tiger_react to not use volumes since they keep old UI pieces around
...
- Fixed an issue with a typo in tiger-cli's default value for the rabbitmq_host variable
...
- Removed the usage of the volume container for `tiger_graphql`
  - Existing volumes for tiger_graphql break new updates since the old volume information is used

## 0.2.4 - 2024-01-28

### Changed

- Refactored to allow multiple kinds of managers for tiger (defaults to `Docker`)
...
- Removed the hard-coded tiger docker network subnet of 172.100.0.0/16. With this update you need to tear down all containers and rebuild so that the new networking applies.
...
### Changed
- Updated the `./tiger-cli status` command to break out a distinction between services in docker compose that aren't running and those not in docker compose
...
- Updated the `./tiger-cli status` command to list out additional services installed but not present in docker-compose
- Updated the `./tiger-cli config service` command to list out just configurations needed for remote agent development
- Added new variable, `tiger_server_dynamic_ports_bind_localhost_only`, specifically to control if dynamic ports are bound to localhost or not, separate from tiger_server

## v0.1.19 - 2023-06-07

### Changed

- Updated docker-compose to bind the Dynamic ports for tiger to localhost if `tiger_server_bind_localhost_only` is set to true

## v0.1.17 - 2023-06-02

### Changed

- Updated tiger_jupyter docker-compose entry to keep the `deploy` key and all sub keys 
- Updated health check for tiger_nginx to use curl instead of wget

## v0.1.15 - 2023-05-17

### Changed

- re-building tiger service containers will re-generate their docker-compose file entries first
...
- Fixed an issue where installing another services after tiger_sync would uninstall tiger_sync
...
- Updated tiger's env to take in configuration channels for the various webhook types and mirror it to containers

## 0.1.8 - 2023-04-20

### changed

- tiger_sync installation pointed to normal service installation instead of tiger_sync install
