...
func (t *pushC2Server) StartPushC2StreamingOneToMany(stream services.PushC2_StartPushC2StreamingOneToManyServer) error {
	...
	rabbitmqProcessAgentMessageTotigerChannel := make(chan RabbitMQProcessAgentMessageFromPushC2, 1)
	disconnectChannel := make(chan bool)
	newPushConnectionChannel := t.GetRabbitMqProcessAgentMessageChannel()
	// make channel with 2 in case something happens and disconnects the client before tiger responds
	// don't want to block tiger accidentally
	newPushConnectionChannel <- PushC2ServerConnected{
		PushC2MessagesTotiger:   rabbitmqProcessAgentMessageTotigerChannel,
	...
	for {
		...
		newPushMessageFromtigerChannel, err := t.addNewPushC2OneToManyClient(c2ProfileName)
		if err != nil {
			// tiger can't keep track of the new connection for some reason, abort it
		...
		go func() {
			...
			for {
				...
				// send the agent message along to tiger for processing and catch response
				select {
				case rabbitmqProcessAgentMessageTotigerChannel <- RabbitMQProcessAgentMessageFromPushC2{
				...
				var fromtigerResponse RabbitMQProcessAgentMessageFromPushC2Response
				select {
				case fromtigerResponse = <-rabbitmqProcessAgentMessageResponseChannel:
				case <-time.After(t.GetChannelTimeout()):
					err = errors.New("timeout receiving msg from tiger to channel")
					logging.LogError(err, "gRPC stream connection needs to exit due to timeouts")
					failedReadFromAgent <- true
					return
				}
				go updatePushC2OneToManyLastCheckinConnectTimestamp(fromtigerResponse, len(fromAgent.Base64Message) > 0, c2ProfileName)
				if fromtigerResponse.Err != nil {
					// tiger encountered an error with the first message from the agent, return error and wait for the next
					err = stream.Send(&services.PushC2MessageFromtiger{
						Success:    false,
						Error:      fromtigerResponse.Err.Error(),
						Message:    nil,
						TrackingID: fromtigerResponse.TrackingID,
					})
					if err != nil {
				...
				err = stream.Send(&services.PushC2MessageFromtiger{
					Success:    true,
					Error:      "",
					Message:    fromtigerResponse.Message,
					TrackingID: fromtigerResponse.TrackingID,
				})
				if err != nil {
					// we failed to send the message back to the agent, bail
		...
		for {
			...
			//logging.LogInfo("pushc2 stream waiting to hear from tiger to send to agent")
			select {
			...
			case <-stream.Context().Done():
				...
				go updatePushC2OneToManyLastCheckinDisconnectTimestamp(c2ProfileName, "")
				return errors.New(fmt.Sprintf("c2 disconnected: %s", c2ProfileName))
			case msgToSend, ok := <-newPushMessageFromtigerChannel:
				if !ok {
				...
				//logging.LogDebug("sending message from tiger to agent")
...
func updatePushC2OneToManyLastCheckinConnectTimestamp(fromtigerResponse RabbitMQProcessAgentMessageFromPushC2Response,
	base64Encoded bool,
	...
	if fromtigerResponse.OuterUuidIsCallback {
		callbackUUID = fromtigerResponse.OuterUuid
		callback := databaseStructs.Callback{}
		callbackId, ok = callbackUUIDToIDMap[fromtigerResponse.OuterUuid]
		if !ok {
			err := database.DB.Get(&callback, `SELECT id, operation_id FROM callback WHERE agent_callback_id=$1`, fromtigerResponse.OuterUuid)
			if err != nil {
				logging.LogError(err, "Failed to find callback info to update connection time")
				return
			}
			callbackUUIDToIDMap[fromtigerResponse.OuterUuid] = callback.ID
	...
	} else if fromtigerResponse.NewCallbackUUID != "" {
		callbackUUID = fromtigerResponse.NewCallbackUUID
		callbackId, ok = callbackUUIDToIDMap[fromtigerResponse.NewCallbackUUID]
		if !ok {
			callback := databaseStructs.Callback{}
			err := database.DB.Get(&callback, `SELECT id, operation_id FROM callback WHERE agent_callback_id=$1`, fromtigerResponse.NewCallbackUUID)
			if err != nil {
				logging.LogError(err, "Failed to find callback info to update connection time")
				return
			}
			callbackUUIDToIDMap[fromtigerResponse.NewCallbackUUID] = callback.ID
			callbackIDToOperationIDMap[callback.ID] = callback.OperationID
			callbackId = callback.ID
			operationId = callback.OperationID
		} else {
	...
	if _, ok = c2ProfileToCallbackIDsMap[c2ProfileName][callbackId]; !ok {
		...
		c2ProfileToCallbackIDsMap[c2ProfileName][callbackId] = cachedCallbackInfo{
			...
			TrackingID:    fromtigerResponse.TrackingID,
			AgentUUIDSize: fromtigerResponse.AgentUUIDSize,
		}
	}
