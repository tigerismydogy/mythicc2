...
func (t *translationContainerServer) TranslateFromtigerToCustomFormat(stream services.TranslationContainer_TranslateFromtigerToCustomFormatServer) error {
	clientName := ""
	...
	if initial, err := stream.Recv(); err == io.EOF {
		...
		if getMessageToSend, sendBackMessageResponse, err := t.addNewtigerToCustomClient(clientName); err != nil {
			...
			for {
				...
				case <-stream.Context().Done():
					...
					t.SettigerToCustomChannelExited(clientName)
				...
				case msgToSend, ok := <-getMessageToSend:
					if !ok {
						...
						if err = stream.Send(&msgToSend); err != nil {
							...
							select {
							case sendBackMessageResponse <- services.TrtigerC2ToCustomMessageFormatMessageResponse{
								Success:                  false,
								Error:                    err.Error(),
