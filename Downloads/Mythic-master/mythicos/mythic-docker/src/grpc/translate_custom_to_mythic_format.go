...
func (t *translationContainerServer) TranslateFromCustomTotigerFormat(stream services.TranslationContainer_TranslateFromCustomTotigerFormatServer) error {
	clientName := ""
	...
	if initial, err := stream.Recv(); err == io.EOF {
		...
		if getMessageToSend, sendBackMessageResponse, err := t.addNewCustomTotigerClient(clientName); err != nil {
			...
			for {
				...
				case <-stream.Context().Done():
					...
					t.SetCustomTotigerChannelExited(clientName)
				...
				case msgToSend, ok := <-getMessageToSend:
					if !ok {
						...
						if err = stream.Send(&msgToSend); err != nil {
							...
							select {
							case sendBackMessageResponse <- services.TrCustomMessageTotigerC2FormatMessageResponse{
								Success:                  false,
								Error:                    err.Error(),
