...
func (t *pushC2Server) StartPushC2Streaming(stream services.PushC2_StartPushC2StreamingServer) error {
	...
	for {
		...
		var fromtiger RabbitMQProcessAgentMessageFromPushC2Response
		select {
		case fromtiger = <-rabbitmqProcessAgentMessageResponseChannel:
		case <-time.After(t.GetChannelTimeout()):
			err = errors.New("timeout receiving msg from tiger to channel")
			logging.LogError(err, "gRPC stream connection needs to exit due to timeouts")
			return err
		}
		if fromtiger.Err != nil {
			...
			err = stream.Send(&services.PushC2MessageFromtiger{
				Success:    false,
				Error:      fromtiger.Err.Error(),
				Message:    nil,
				TrackingID: fromtiger.TrackingID,
			})
			if err != nil {
		...
		if fromtiger.NewCallbackUUID != "" {
			// we just finished staging and got a new callback
			callbackUUID = fromtiger.NewCallbackUUID
		} else if fromtiger.OuterUuidIsCallback {
			// we already have an existing callback sending messages
			callbackUUID = fromtiger.OuterUuid
		} else {
		...
		newPushMessageFromtigerChannel, err := t.addNewPushC2Client(callback.ID, callbackUUID, base64Message != nil, c2ProfileName, fromtiger.AgentUUIDSize)
		if err != nil {
