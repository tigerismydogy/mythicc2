...
type tigerRPCFileSearchMessage struct {
...
type tigerRPCFileSearchMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_FILE_SEARCH,
		RoutingKey: tiger_RPC_FILE_SEARCH,
		Handler:    processtigerRPCFileSearch,
	})
}

func tigerRPCFileSearch(input tigerRPCFileSearchMessage) tigerRPCFileSearchMessageResponse {
	response := tigerRPCFileSearchMessageResponse{
		Success: false,
	}
	var callbackId int
	var operationId int
	if input.AgentFileID != "" {
		...
		if err := database.DB.Get(&fileMeta, `SELECT 
			...
			logging.LogError(err, "Failed to get specified file in tigerRPCFileSearch")
		...
		if err := database.DB.Get(&callback, `SELECT operation_id, id FROM callback WHERE id=$1`, callback.ID); err != nil {
			logging.LogError(err, "Failed to get operation in tigerRPCFileSearch")
			response.Error = err.Error()
			return response
		} else {
		...
		if err := database.DB.Get(&task, `SELECT operation_id, callback_id FROM task WHERE id=$1`, task.ID); err != nil {
			logging.LogError(err, "Failed to get task from tiger")
			response.Error = err.Error()
			return response
		} else {
	...
	if input.Comment != "" {
		...
		if err := database.DB.Select(&files, searchString, searchParameters...); err != nil {
			response.Error = err.Error()
			logging.LogError(err, "Failed to get files by comment in tigerRPCFileSearch")
			return response
		}
	} else {
		if input.Filename == "" {
		...
		if err := database.DB.Select(&files, searchString, searchParameters...); err != nil {
			response.Error = err.Error()
			logging.LogError(err, "Failed to get files by filename in tigerRPCFileSearch")
			return response
		}
	}
...
func processtigerRPCFileSearch(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCFileSearchMessage{}
	responseMsg := tigerRPCFileSearchMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCFileSearch(incomingMessage)
	}
	return responseMsg
}
