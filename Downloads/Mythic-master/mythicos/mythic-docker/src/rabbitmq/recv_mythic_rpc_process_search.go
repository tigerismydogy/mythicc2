...
type tigerRPCProcessSearchMessage struct {
	TaskID        int                               `json:"task_id"` //required
	SearchProcess tigerRPCProcessSearchProcessData `json:"process"`
}
type tigerRPCProcessSearchMessageResponse struct {
	Success   bool                                `json:"success"`
	Error     string                              `json:"error"`
	Processes []tigerRPCProcessSearchProcessData `json:"processes"`
}
type tigerRPCProcessSearchProcessData struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_PROCESS_SEARCH,
		RoutingKey: tiger_RPC_PROCESS_SEARCH,
		Handler:    processtigerRPCProcessSearch,
	})
}

// Endpoint: tiger_RPC_PROCESS_SEARCH
func tigerRPCProcessSearch(input tigerRPCProcessSearchMessage) tigerRPCProcessSearchMessageResponse {
	response := tigerRPCProcessSearchMessageResponse{
		Success:   false,
		Processes: []tigerRPCProcessSearchProcessData{},
	}
	...
	WHERE task.id=$1`, input.TaskID); err != nil {
		...
		searchString := `SELECT * FROM tigertree WHERE operation_id=:operation_id AND tree_type='process' `
...
func processtigerRPCProcessSearch(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCProcessSearchMessage{}
	responseMsg := tigerRPCProcessSearchMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCProcessSearch(incomingMessage)
	}
	return responseMsg
}
