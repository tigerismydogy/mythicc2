...
func (cache *cachedUUIDInfo) getAllKeys() []tigerCrypto.CryptoKeys {
	switch cache.UUIDType {
	case UUIDTYPECALLBACK:
		return []tigerCrypto.CryptoKeys{
			{
	...
	case UUIDTYPEPAYLOAD:
		options := []tigerCrypto.CryptoKeys{}
		if cache.C2EncKey != nil {
			options = append(options, tigerCrypto.CryptoKeys{
	...
	for _, keyOpt := range keyOptions {
		...
		case "aes256_hmac":
			if action == "decrypt" {
				...
				if modified, err = tigerCrypto.DecryptAES256HMAC(*keyToUse, agentMessage); err == nil {
				...
				if modified, err := tigerCrypto.EncryptAES256HMAC(*keyToUse, agentMessage); err == nil {
...
// this is what tiger is sending back in the delegates field
type delegateMessageResponse struct {
	...
	C2ProfileName string `json:"c2_profile" mapstructure:"c2_profile" xml:"c2_profile"`
	tigerUuid    string `json:"tiger_uuid,omitempty" mapstructure:"tiger_uuid,omitempty" xml:"tiger_uuid,omitempty"`
...
func recursiveProcessAgentMessage(agentMessageInput AgentMessageRawInput) recursiveProcessAgentMessageResponse {
	...
	// 1. Get message
	if utils.tigerConfig.DebugAgentMessage {
	...
	if err != nil {
		...
		errorMessage := fmt.Sprintf("Failed to correlate UUID, %s, to something tiger knows.\n", messageUUID.String())
		errorMessage += fmt.Sprintf("%s is likely a Callback or Payload from a tiger instance that was deleted or had the database reset.\n", messageUUID.String())
	...
	if err != nil {
		logging.LogError(err, "Failed to process message from tiger")
		instanceResponse.Err = err
		return instanceResponse
	}
			...
			for _, delegate := range delegates {
				...
				if delegateResponse := recursiveProcessAgentMessage(currentDelegateMessage); delegateResponse.Err != nil {
					...
					if delegateResponse.NewCallbackUUID != "" && delegateResponse.NewCallbackUUID != delegate.SuppliedUuid {
						newResponse.tigerUuid = delegateResponse.NewCallbackUUID
						...
						if delegateResponse.OuterUuid != "" && delegateResponse.OuterUuid != delegate.SuppliedUuid {
							newResponse.tigerUuid = delegateResponse.OuterUuid
...
func LookupEncryptionData(c2profile string, messageUUID string, updateCheckinTime bool) (*cachedUUIDInfo, error) {
	...
	if err := database.DB.Get(&databaseC2Profile, `SELECT id, "name", is_p2p FROM c2profile
		...
		errorMessage += fmt.Sprintf("This could be from a C2 Profile forwarding traffic to tiger that tiger isn't tracking.\n")
	...
	if err := database.DB.Get(&callback, `SELECT
		...
		payloadtype.tiger_encrypts "payload.payloadtype.tiger_encrypts",
		...
		newCache.tigerEncrypts = callback.Payload.Payloadtype.tigerEncrypts
		...
		payloadtype.tiger_encrypts "payloadtype.tiger_encrypts",
		...
		newCache.tigerEncrypts = payload.Payloadtype.tigerEncrypts
		...
		newCache.tigerEncrypts = stager.Payload.Payloadtype.tigerEncrypts
	...
	if uuidInfo.tigerEncrypts {
		if uuidInfo.TranslationContainerName == "" {
			...
			if decrypted, err := uuidInfo.IterateAndAct(agentMessage, "decrypt"); err != nil {
				...
				return nil, err
			} else if convertedResponse, err := RabbitMQConnection.SendTrRPCCustomMessageTotigerC2(TrCustomMessageTotigerC2FormatMessage{
				TranslationContainerName: uuidInfo.TranslationContainerName,
				C2Name:                   uuidInfo.C2ProfileName,
				Message:                  decrypted,
				UUID:                     uuidInfo.UUID,
				tigerEncrypts:           uuidInfo.tigerEncrypts,
				CryptoKeys:               uuidInfo.getAllKeys(),
			}); err != nil {
				//logging.LogError(err, "Failed to send response to translate custom message to tiger C2")
				...
				//	"c2_to_tiger_"+uuidInfo.TranslationContainerName, database.MESSAGE_LEVEL_WARNING)
				return nil, err
		...
		if uuidInfo.TranslationContainerName == "" {
			...
			if convertedResponse, err := RabbitMQConnection.SendTrRPCCustomMessageTotigerC2(TrCustomMessageTotigerC2FormatMessage{
				TranslationContainerName: uuidInfo.TranslationContainerName,
				C2Name:                   uuidInfo.C2ProfileName,
...
func EncryptMessage(uuidInfo *cachedUUIDInfo, outerUUID string, agentMessage map[string]interface{}, agentUUIDLength int, shouldBase64Encode bool) ([]byte, error) {
	...
	if uuidInfo.tigerEncrypts {
		...
		convertedResponse, err := RabbitMQConnection.SendTrRPCtigerC2ToCustomMessage(TrtigerC2ToCustomMessageFormatMessage{
			TranslationContainerName: uuidInfo.TranslationContainerName,
			C2Name:                   uuidInfo.C2ProfileName,
			Message:                  agentMessage,
			UUID:                     uuidInfo.UUID,
		...
		if err != nil {
			...
			//	"tiger_to_c2_"+uuidInfo.TranslationContainerName, database.MESSAGE_LEVEL_WARNING)
			return nil, err
		}
		if !convertedResponse.Success {
			logging.LogError(errors.New(convertedResponse.Error), "Failed to have translation container process message from tiger->Custom C2")
			go SendAllOperationsMessage(fmt.Sprintf("Failed to have translation container process message from tiger->Custom C2: %s\n%s", uuidInfo.TranslationContainerName, convertedResponse.Error), uuidInfo.OperationID,
				"tiger_to_c2_"+uuidInfo.TranslationContainerName, database.MESSAGE_LEVEL_WARNING)
			return nil, errors.New(convertedResponse.Error)
		}
		...
		if uuidInfo.TranslationContainerName == "" {
			// tiger doesn't encrypt, but there's no translation container, so just return it
			jsonBytes, err := marshalMessageForAgentFormat(uuidInfo, agentMessage)
			//jsonBytes, err := json.Marshal(agentMessage)
			if err != nil {
