...
type tigerRPCCallbackDecryptBytesMessage struct {
...
type tigerRPCCallbackDecryptBytesMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_CALLBACK_DECRYPT_BYTES,
		RoutingKey: tiger_RPC_CALLBACK_DECRYPT_BYTES,
		Handler:    processtigerRPCCallbackDecryptBytes,
	})
}

// Endpoint: tiger_RPC_CALLBACK_DECRYPT_BYTES
func tigerRPCCallbackDecryptBytes(input tigerRPCCallbackDecryptBytesMessage) tigerRPCCallbackDecryptBytesMessageResponse {
	response := tigerRPCCallbackDecryptBytesMessageResponse{
		Success: false,
	}
...
func CallbackDecryptMessage(agentCallbackUUID string, message *[]byte, includesUUID bool, isBase64Encoded bool) ([]byte, error) {
	...
	WHERE id=$1`, callback.Payload.PayloadTypeID); err != nil {
		...
		//logging.LogInfo("Decrypt message", "tiger encrypts", true)
		var cipherText []byte
		var err error
		if callback.DecKey == nil {
			//logging.LogInfo("Decrypt message", "dec key", nil, "agent message", agentMessage)
			return agentMessage, nil
		} else if cipherText, err = tigerCrypto.DecryptAES256HMAC(*callback.DecKey, agentMessage); err != nil {
...
func processtigerRPCCallbackDecryptBytes(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCCallbackDecryptBytesMessage{}
	responseMsg := tigerRPCCallbackDecryptBytesMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCCallbackDecryptBytes(incomingMessage)
	}
	return responseMsg
}
