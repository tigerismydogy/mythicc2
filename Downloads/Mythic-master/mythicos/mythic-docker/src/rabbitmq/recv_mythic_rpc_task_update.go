...
type tigerRPCTaskUpdateMessage struct {
...
type tigerRPCTaskUpdateMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_TASK_UPDATE,     // swap out with queue in rabbitmq.constants.go file
		RoutingKey: tiger_RPC_TASK_UPDATE,     // swap out with routing key in rabbitmq.constants.go file
		Handler:    processtigerRPCTaskUpdate, // points to function that takes in amqp.Delivery and returns interface{}
	})
}

// tiger_RPC_OBJECT_ACTION - Say what the function does
func tigerRPCTaskUpdate(input tigerRPCTaskUpdateMessage) tigerRPCTaskUpdateMessageResponse {
	response := tigerRPCTaskUpdateMessageResponse{
		Success: false,
	}
...
func processtigerRPCTaskUpdate(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCTaskUpdateMessage{}
	responseMsg := tigerRPCTaskUpdateMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCTaskUpdate(incomingMessage)
	}
	return responseMsg
}
