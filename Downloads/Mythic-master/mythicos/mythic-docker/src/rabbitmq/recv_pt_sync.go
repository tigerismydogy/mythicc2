...
type PayloadType struct {
	...
	tigerEncryptsData           bool             `json:"tiger_encrypts"`
...
func payloadTypeSync(in PayloadTypeSyncMessage) error {
	...
	if err := database.DB.Get(&payloadtype, `SELECT * FROM payloadtype WHERE "name"=$1`, in.PayloadType.Name); err != nil {
		...
		payloadtype.tigerEncrypts = in.PayloadType.tigerEncryptsData
		payloadtype.Note = in.PayloadType.Description
		payloadtype.SupportedOs = GettigerJSONArrayFromStruct(in.PayloadType.SupportedOS)
		...
		if statement, err := database.DB.PrepareNamed(`INSERT INTO payloadtype 
			("name",author,container_running,file_extension,tiger_encrypts,note,supported_os,supports_dynamic_loading,wrapper,agent_type,message_format) 
			VALUES (:name, :author, :container_running, :file_extension, :tiger_encrypts, :note, :supported_os, :supports_dynamic_loading, :wrapper,:agent_type, :message_format) 
			RETURNING id`,
		); err != nil {
		...
		_, err = database.DB.NamedExec(`UPDATE payloadtype SET 
			author=:author, container_running=:container_running, file_extension=:file_extension, tiger_encrypts=:tiger_encrypts,
...
func updatePayloadTypeCommands(in PayloadTypeSyncMessage, payloadtype databaseStructs.Payloadtype) error {
			...
			if err = rows.StructScan(&databaseCommand); err != nil {
				...
				for _, newCommand := range syncingCommands {
					if newCommand.Name == databaseCommand.Cmd {
						...
						databaseCommand.SupportedUiFeatures = GettigerJSONArrayFromStruct(newCommand.SupportedUIFeatures)
						...
						databaseCommand.Attributes = GettigerJSONTextFromStruct(attributes)
	...
	for _, newCommand := range syncingCommands {
		...
		databaseCommand = databaseStructs.Command{
			...
			SupportedUiFeatures: GettigerJSONArrayFromStruct(newCommand.SupportedUIFeatures),
					...
					if newParameter.Name == databaseParameter.Name {
						...
						for _, newParameterGroup := range newParameter.ParameterGroupInformation {
							...
							if newParameterGroup.GroupName == databaseParameter.ParameterGroupName {
								...
								databaseParameter.Description = newParameter.Description
								databaseParameter.Choices = GettigerJSONArrayFromStruct(newParameter.Choices)
								if defaultVal, err := getSyncToDatabaseValueForDefaultValue(newParameter.ParameterType, newParameter.DefaultValue, newParameter.Choices); err != nil {
								...
								databaseParameter.SupportedAgents = GettigerJSONArrayFromStruct(newParameter.SupportedAgents)
								...
								databaseParameter.ChoiceFilterByCommandAttributes = GettigerJSONTextFromStruct(newParameter.FilterCommandChoicesByCommandAttributes)
								databaseParameter.DynamicQueryFunction = newParameter.DynamicQueryFunctionName
								databaseParameter.SupportedAgentBuildParameters = GettigerJSONTextFromStruct(newParameter.SupportedAgentBuildParameters)
		...
		for _, newParameterGroup := range newParameter.ParameterGroupInformation {
			if utils.SliceContains(updatedAndDeletedParameters, newParameter.Name+newParameterGroup.GroupName) {
				...
				databaseParameter = databaseStructs.Commandparameters{
					...
					Description:              newParameter.Description,
					Choices:                  GettigerJSONArrayFromStruct(newParameter.Choices),
					SupportedAgents:          GettigerJSONArrayFromStruct(newParameter.SupportedAgents),
