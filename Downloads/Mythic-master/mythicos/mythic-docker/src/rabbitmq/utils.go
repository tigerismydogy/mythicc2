...
func getSyncToDatabaseValueForChoices(parameterType string, choices []string, dictionaryChoices []ParameterDictionary) (databaseStructs.tigerJSONArray, error) {
	...
	case BUILD_PARAMETER_TYPE_CHOOSE_MULTIPLE:
		return GettigerJSONArrayFromStruct(choices), nil
	case BUILD_PARAMETER_TYPE_DICTIONARY:
		return GettigerJSONArrayFromStruct(dictionaryChoices), nil
...
func GettigerJSONTextFromStruct(input interface{}) databaseStructs.tigerJSONText {
	newType := databaseStructs.tigerJSONText{}
	if err := newType.Scan(input); err != nil {
		logging.LogError(err, "Failed to marshal struct into databaseStructs.tigerJSONText")
	}
	return newType
}
func GettigerJSONArrayFromStruct(input interface{}) databaseStructs.tigerJSONArray {
	newType := databaseStructs.tigerJSONArray{}
	if err := newType.Scan(input); err != nil {
		logging.LogError(err, "Failed to marshal struct into databaseStructs.tigerJSONArray")
	}
	return newType
}
...
func CheckAndProcessTaskCompletionHandlers(taskId int) {
	...
	if task.CompletedCallbackFunction != "" && !task.CompletedCallbackFunctionCompleted {
		...
		//	if responseCreate, err := tigerrpc.SendtigerRPCTaskCreateSubtask(tigerrpc.tigerRPCTaskCreateSubtaskMessage{
