...
import (
	...
	"github.com/its-a-feature/tiger/database/enums/InteractiveTask"
...
type agentMessagePostResponseDownload struct {
	// Transfer a file from agent -> tiger
...
type agentMessagePostResponseUpload struct {
	// Transfer a file from tiger -> Agent
	ChunkSize *int                   `json:"chunk_size,omitempty" mapstructure:"chunk_size,omitempty" xml:"chunk_size,omitempty"`
	...
	*/
	...
	if err := mapstructure.Decode(incoming, &agentMessage); err != nil {
		...
		for _, agentResponse := range agentMessage.Responses {
			tigerResponse := map[string]interface{}{
			...
			WHERE task.agent_task_id=$1`, currentTask.AgentTaskID); err != nil {
				...
				tigerResponse["status"] = "error"
				tigerResponse["error"] = "Failed to find task"
				responses = append(responses, tigerResponse)
				continue
			} else {
				...
				if agentResponse.Download != nil {
					if newFileID, err := handleAgentMessagePostResponseDownload(currentTask, agentResponse); err != nil {
						tigerResponse["status"] = "error"
						tigerResponse["error"] = err.Error()
					} else {
						tigerResponse["file_id"] = newFileID
					}
					if agentResponse.Download.ChunkNum != nil {
						tigerResponse["chunk_num"] = *agentResponse.Download.ChunkNum
					}
				...
				if agentResponse.Upload != nil {
					...
					} else if err := mapstructure.Decode(uploadResponse, &tigerResponse); err != nil {
						tigerResponse["status"] = "error"
						tigerResponse["error"] = err.Error()
				...
				reflectBackOtherKeys(&tigerResponse, &agentResponse.Other)
				...
				WHERE id=:id`, currentTask); err != nil {
					...
					tigerResponse["status"] = "error"
					tigerResponse["error"] = "Failed to update task"
					responses = append(responses, tigerResponse)
				} else {
					responses = append(responses, tigerResponse)
...
func handleAgentMessagePostResponseRemovedFiles(task databaseStructs.Task, removedFiles *[]agentMessagePostResponseRemovedFiles) error {
	...
	for _, rmData := range *removedFiles {
		...
		if _, err := database.DB.Exec(`UPDATE tigertree SET
		  deleted=true
...
func listenForWriteDownloadChunkToLocalDisk() {
	...
	for {
		...
		case agentResponse := <-writeDownloadChunkToDiskChan:
			f, err := os.OpenFile(agentResponse.LocaltigerPath, os.O_RDWR|os.O_CREATE, 0644)
			if err != nil {
...
func handleAgentMessagePostResponseDownload(task databaseStructs.Task, agentResponse agentMessagePostResponse) (string, error) {
	// might need to return a file_id if we're initially registering a file for transfer from agent to tiger
	// two stages:
	/*
	...
	*/
	...
	if agentResponse.Download.FileID != nil && *agentResponse.Download.FileID != "" {
		...
		if err := database.DB.Get(&fileMeta, `SELECT 
			...
			if !fileMeta.Complete {
				if agentResponse.Download.ChunkData != nil && len(*agentResponse.Download.ChunkData) > 0 {
					...
					//logging.LogDebug("0. about to have tiger write to disk", "chunk num", *agentResponse.Download.ChunkNum, "byte sample", string(base64DecodedFileData[:10]))
		...
		if statement, err := database.DB.PrepareNamed(`INSERT INTO filemeta 
			...
			go EmitFileLog(fileMeta.ID)
			go addFileMetaTotigerTree(task, fileMeta)
			return fileMeta.AgentFileID, nil
		}
	} else {
...
func handleAgentMessagePostResponseUpload(task databaseStructs.Task, agentResponse agentMessagePostResponse) (agentMessagePostResponseUploadResponse, error) {
	// transferring a file from tiger to the agent.
	...
	if agentResponse.Upload.FileID != nil && *agentResponse.Upload.FileID != "" {
		...
		if err := database.DB.Get(&fileMeta, `SELECT 
			*
			...
			if !fileMeta.Complete {
				logging.LogError(nil, "Trying to upload a file to an agent that isn't fully on tiger's server yet")
				go SendAllOperationsMessage(fmt.Sprintf("Failed to transfer file to agent - not completely uploaded to tiger yet: %s\n", *agentResponse.Upload.FileID), task.OperationID, "", database.MESSAGE_LEVEL_WARNING)
				return uploadResponse, errors.New("trying to upload a file to an agent that isn't fully on tiger's server yet")
			} else if fileMeta.Deleted {
				logging.LogError(nil, "Trying to upload a file to an agent that is deleted")
				go SendAllOperationsMessage(fmt.Sprintf("Failed to transfer file to agent - file was deleted from tiger: %s\n", *agentResponse.Upload.FileID), task.OperationID, "", database.MESSAGE_LEVEL_WARNING)
	...
	if agentResponse.Upload.FullPath != nil && *agentResponse.Upload.FullPath != "" {
		if filePieces, err := utils.SplitFilePathGetHost(*agentResponse.Upload.FullPath, "", []string{}); err != nil {
			...
			RETURNING id`); err != nil {
					...
					go associateFileMetaWithtigerTree(filePieces, newFileMeta, task)
				}
			} else {
				...
				if _, err := database.DB.NamedExec(`UPDATE filemeta 
					...
					go EmitFileLog(fileMeta.ID)
					go associateFileMetaWithtigerTree(filePieces, fileMeta, task)
				}
			}
		}
	}
...
func associateFileMetaWithtigerTree(pathData utils.AnalyzedPath, fileMeta databaseStructs.Filemeta, task databaseStructs.Task) {
	resolveAndCreateParentPathsForTreeNode(pathData, task, databaseStructs.TREE_TYPE_FILE)
	newTree := databaseStructs.tigerTree{
		Host:            pathData.Host,
		TaskID:          task.ID,
		OperationID:     task.OperationID,
	...
	newTree.Metadata = GettigerJSONTextFromStruct(fileMetaData)
	...
	// now that we know the tigertree entry exists, associate this filemeta with it
	fileMeta.tigerTreeID.Valid = true
	fileMeta.tigerTreeID.Int64 = int64(newTree.ID)
	if _, err := database.DB.NamedExec(`UPDATE filemeta SET tigertree_id=:tigertree_id WHERE id=:id`, fileMeta); err != nil {
		logging.LogError(err, "Failed to associate filemeta with tigertree ")
					...
					if fileBrowser.Files != nil {
						...
						for _, newEntry := range *fileBrowser.Files {
							if bytes.Equal([]byte(newEntry.Name), existingEntry.Name) {
								...
								newTreeChild := databaseStructs.tigerTree{
									Host:            pathData.Host,
									TaskID:          task.ID,
									OperationID:     task.OperationID,
								...
								newTreeChild.Metadata = GettigerJSONTextFromStruct(fileMetaData)
			...
			for _, existingEntry := range existingTreeEntries {
				...
				for _, newEntry := range *processes {
					if strconv.Itoa(newEntry.ProcessID) == string(existingEntry.FullPath) &&
						...
						newTree.Metadata = GettigerJSONTextFromStruct(metadata)
						newTree.CallbackID.Valid = true
						newTree.CallbackID.Int64 = int64(task.Callback.ID)
						createTreeNode(&newTree)
					}
...
func getParentPathFullPathName(pathData utils.AnalyzedPath, endIndex int, treeType string) (string, string, string) {
	...
	default:
		logging.LogError(nil, "Unknown tigertree type", "tree type", treeType)
		return "", "", ""
	}
}
func updateTreeNode(treeNode databaseStructs.tigerTree) {
	if _, err := database.DB.NamedExec(`UPDATE tigertree SET
        success=:success, deleted=:deleted, metadata=tigertree.metadata || :metadata, task_id=:task_id
		WHERE id=:id
`, treeNode); err != nil {
		logging.LogError(err, "Failed to update tree node")
	}
}
func deleteTreeNode(treeNode databaseStructs.tigerTree, cascade bool) {
	if cascade {
...
func createTreeNode(treeNode *databaseStructs.tigerTree) {
	if len(treeNode.Name) == 0 {
		logging.LogError(nil, "Can't create file browser entry with empty name")
		return
	}
	if statement, err := database.DB.PrepareNamed(`INSERT INTO tigertree
		...
		task_id=:task_id, "name"=:name, parent_path=:parent_path, can_have_children=:can_have_children,
		    metadata=tigertree.metadata || :metadata, os=:os, "timestamp"=now(), deleted=false
		    RETURNING id`); err != nil {
		logging.LogError(err, "Failed to create new tigertree statement")
	} else if err = statement.Get(&treeNode.ID, treeNode); err != nil {
		logging.LogError(err, "Failed to create new tigertree entry")
	}
}
func addFileMetaTotigerTree(task databaseStructs.Task, newFile databaseStructs.Filemeta) {
	fileBrowser := databaseStructs.tigerTree{}
	if newFile.FullRemotePath == nil || len(newFile.FullRemotePath) == 0 {
		logging.LogError(nil, "Trying to addFileMeta to tiger's Tree, but no FullRemotePath provided")
		return
	}
	if err := database.DB.Get(&fileBrowser, `SELECT id FROM tigertree WHERE
		...
		if pathData, err := utils.SplitFilePathGetHost(string(newFile.FullRemotePath), "", []string{}); err != nil {
			...
			associateFileMetaWithtigerTree(pathData, newFile, task)
			/*
				resolveAndCreateParentPathsForTreeNode(pathData, task, databaseStructs.TREE_TYPE_FILE)
				if err := database.DB.Get(&fileBrowser, `SELECT id FROM tigertree WHERE
					...
					logging.LogError(err, "Failed to find, create, and then fine tiger tree data for newly downloaded file")
				} else {
					newFile.tigerTreeID.Int64 = int64(fileBrowser.ID)
					newFile.tigerTreeID.Valid = true
					if _, err := database.DB.NamedExec(`UPDATE filemeta SET tigertree_id=:tigertree_id WHERE id=:id`, newFile); err != nil {
						logging.LogError(err, "Failed to update file meta with tiger tree id")
					} else if _, err := database.DB.Exec(`UPDATE tigertree SET can_have_children=false WHERE id=$1`, fileBrowser.ID); err != nil {
