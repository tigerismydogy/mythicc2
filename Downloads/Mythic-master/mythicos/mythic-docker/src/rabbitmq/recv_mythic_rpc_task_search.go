...
type tigerRPCTaskSearchMessage struct {
...
type tigerRPCTaskSearchMessageResponse struct {
...
func init() {
	RabbitMQConnection.AddRPCQueue(RPCQueueStruct{
		...
		Queue:      tiger_RPC_TASK_SEARCH,     // swap out with queue in rabbitmq.constants.go file
		RoutingKey: tiger_RPC_TASK_SEARCH,     // swap out with routing key in rabbitmq.constants.go file
		Handler:    processtigerRPCTaskSearch, // points to function that takes in amqp.Delivery and returns interface{}
	})
}

// tiger_RPC_OBJECT_ACTION - Say what the function does
func tigerRPCTaskSearch(input tigerRPCTaskSearchMessage) tigerRPCTaskSearchMessageResponse {
	response := tigerRPCTaskSearchMessageResponse{
		Success: false,
	}
...
func processtigerRPCTaskSearch(msg amqp.Delivery) interface{} {
	incomingMessage := tigerRPCTaskSearchMessage{}
	responseMsg := tigerRPCTaskSearchMessageResponse{
		Success: false,
	}
	if err := json.Unmarshal(msg.Body, &incomingMessage); err != nil {
		...
		return tigerRPCTaskSearch(incomingMessage)
	}
	return responseMsg
}
