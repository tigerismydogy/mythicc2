...
func CookieAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.Request.Header.Add("tigerSource", "cookie")
		c.Next()
	}
}
...
func IPBlockMiddleware() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if ipAddr == nil {
			...
			for _, subnet := range utils.tigerConfig.AllowedIPBlocks {
		...
		go rabbitmq.SendAllOperationsMessage(fmt.Sprintf("Client IP, %s, not in allowed IP blocks: %v", ipAddr.String(), utils.tigerConfig.AllowedIPBlocks),
