...
func TokenValid(c *gin.Context) error {
	...
	if err != nil {
		...
		c.Request.Header.Add("tigerSource", "scripting")
	...
	if len(tokenString) > 0 {
		_, err = jwt.ParseWithClaims(tokenString, &claims, func(token *jwt.Token) (interface{}, error) {
			...
			return utils.tigerConfig.JWTSecret, nil
		})
		if err != nil {
...
func GenerateJWT(user databaseStructs.Operator, authMethod string) (string, string, int, error) {
	...
	access_token, err := token.SignedString(utils.tigerConfig.JWTSecret)
	if err != nil {
