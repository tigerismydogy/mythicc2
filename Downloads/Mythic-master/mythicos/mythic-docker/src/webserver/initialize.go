...
import (
	...
	"github.com/its-a-feature/tiger/authentication"
	"github.com/its-a-feature/tiger/logging"
	"github.com/its-a-feature/tiger/utils"
	webcontroller "github.com/its-a-feature/tiger/webserver/controllers"
)

func Initialize() *gin.Engine {
	//gin.DisableConsoleColor()
	switch utils.tigerConfig.DebugLevel {
	case "warning":
...
func StartServer(r *gin.Engine) {
	...
	logging.LogInfo("Starting webserver", "host", "0.0.0.0", "port", utils.tigerConfig.ServerPort)
	if err := r.Run(fmt.Sprintf("%s:%d", "0.0.0.0", utils.tigerConfig.ServerPort)); err != nil {
...
func InitializeGinLogger() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if !utils.SliceContains(ignorePaths, param.Path) {
			...
			source := c.GetHeader("tigerSource")
			if source == "" {
				source = "scripting"
			}
...
func setRoutes(r *gin.Engine) {
	...
	r.Use(authentication.IPBlockMiddleware())
	{
		...
		protected.Use(authentication.JwtAuthMiddleware())
		{
			...
			{
				// Only OPERATION_ADMIN and tiger_ADMIN can do these routes
