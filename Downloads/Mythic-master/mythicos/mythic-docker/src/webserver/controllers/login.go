...
func Login(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		...
		c.SetCookie("tiger", accessToken, 60*60*24*2, "/", strings.Split(c.Request.Host, ":")[0], false, false)
...
func GetHasuraClaims(c *gin.Context) {
	/*
		...
		x-hasura-role <-- view mode for current operation
			operator, spectator, operation_admin, tiger_admin
	...
	*/
	...
	if user.Admin {
		hasuraClaims["x-hasura-role"] = "tiger_admin"
	}
