...
func ImportCallbackConfigWebhook(c *gin.Context) {
	...
	payloadtype := databaseStructs.Payloadtype{
		...
		tigerEncrypts: callbackConfig.PayloadType.tigerEncrypts,
		SupportedOs:    rabbitmq.GettigerJSONArrayFromStruct([]string{callbackConfig.Payload.Os}),
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			("name",tiger_encrypts,supported_os,translation_container_id) 
			VALUES (:name, :tiger_encrypts, :supported_os, :translation_container_id) 
			RETURNING id`,
			)
			if err != nil {
...
(agent_callback_id, "user", host, pid, ip, external_ip, process_name, description, registered_payload_id, integrity_level,
 operation_id, crypto_type, enc_key, dec_key, os, architecture, "domain", extra_info, sleep_info, tigertree_groups, operator_id)
 VALUES (:agent_callback_id, :user, :host, :pid, :ip, :external_ip, :process_name, :description, :registered_payload_id,
         ...
         :tigertree_groups, :operator_id) RETURNING id`)
	if err != nil {
