...
// initial structs made with './tables-to-go -u tiger_user -p [password here] -h [ip here] -v -d tiger_db -of output -pn database_structs'
...
func Initialize() {
		...
		if AreDatabaseErrorsEqual(err, UndefinedTable) {
			...
			if err != nil {
				...
				newUser := databaseStructs.Operator{
					Username: utils.tigerConfig.AdminUser,
					Salt:     salt,
					Admin:    true,
					Active:   true,
				}
				newOperation := databaseStructs.Operation{
					Name:    utils.tigerConfig.DefaultOperationName,
					Webhook: utils.tigerConfig.DefaultOperationWebhook,
					Channel: utils.tigerConfig.DefaultOperationChannel,
				}
				newUser.Password = HashUserPassword(newUser, utils.tigerConfig.AdminPassword)
	...
	migrate.SetTable("tiger_server_migration_tracking")
	...
	if err != nil {
		...
		if err2 := DB.Select(&appliedMigrations, `SELECT * FROM tiger_server_migration_tracking`); err2 != nil {
...
func getNewDbConnection() *sqlx.DB {
	for {
		logging.LogInfo("Attempting to connect to database...", "host", utils.tigerConfig.PostgresHost, "port", utils.tigerConfig.PostgresPort)
		conn, err := sqlx.Connect("postgres", fmt.Sprintf("user='%s' password='%s' host='%s' port='%d' dbname='%s' sslmode=disable connect_timeout=10",
			utils.tigerConfig.PostgresUser,
			utils.tigerConfig.PostgresPassword,
			utils.tigerConfig.PostgresHost,
			utils.tigerConfig.PostgresPort,
			utils.tigerConfig.PostgresDB),
		)
		if err != nil {
			logging.LogError(err, "Failed to connect to database", "host", utils.tigerConfig.PostgresHost, "port", utils.tigerConfig.PostgresPort)
