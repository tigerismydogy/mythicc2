    ...
    useQuery(GetCallbacks, {fetchPolicy: "network-only",
        onCompleted: (data) => {
            ...
            const allOperators = data.task.reduce( (prev, cur) => {
                ...
                for(let i = 0; i < callbackData[cur.callback_id].tigertree_groups.length; i++){
                    let curGroup = callbackData[cur.callback_id].tigertree_groups[i];
                ...
                if(callbackData[cur.callback_id].tigertree_groups.length === 0){
                    let currentTaskedUser = callbackData[cur.callback_id].user;
                    if(callbackData[cur.callback_id].integrity_level > 2){
                        currentTaskedUser = "*" + currentTaskedUser;
                    }
    ...
    return (
        <Paper elevation={5} style={{
            ...
            <Table >
                <TableHead>
                    <TableRow>
                        <tigerTableCell>{leftColumnTitle}</tigerTableCell>
                        <tigerTableCell>{rightColumnTitle}</tigerTableCell>
                    </TableRow>
                </TableHead>
                <TableBody >
                    {data.map( (d, index) => (
                        <TableRow hover key={d[leftKey] + index} onClick={() => {onRowClick(d[leftKey])}} style={{cursor: "pointer"}}>
                            <tigerTableCell>{d[leftKey]}</tigerTableCell>
                            <tigerTableCell>{d[rightKey]}</tigerTableCell>
                        </TableRow>
                        )
                    )}
                </TableBody>
