import {tigerSearchTabLabel, tigerTabPanel} from '../../tigerComponents/tigerTabPanel';
...
import {UploadTaskFile} from '../../tigerComponents/tigerFileUpload';
...
`;
const tigertreeFragment = gql`
fragment tigertreeData on tigertree{
    comment
    deleted
    full_path_text
    host
...
query filenameFileBrowserQuery($operation_id: Int!, $filename: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tigertree_aggregate(distinct_on: id, where: {full_path_text: {_ilike: $filename}, host: {_ilike: $host}, operation_id: {_eq: $operation_id}, tree_type: {_eq: "file"}}) {
      aggregate {
        count
      }
    }
    tigertree(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {host: {_ilike: $host}, full_path_text: {_ilike: $filename}, operation_id: {_eq: $operation_id}, tree_type: {_eq: "file"}}) {
      ...tigertreeData
    }
  }
`;
...
query filenameFileBrowserQuery($operation_id: Int!, $comment: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tigertree_aggregate(distinct_on: id, where: {comment: {_ilike: $comment}, host: {_ilike: $host}, operation_id: {_eq: $operation_id}, tree_type: {_eq: "file"}}) {
      aggregate {
        count
      }
    }
    tigertree(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {host: {_ilike: $host}, comment: {_ilike: $comment}, operation_id: {_eq: $operation_id}, tree_type: {_eq: "file"}}) {
      ...tigertreeData
    }
  }
`;
...
query tagFileBrowserQuery($tag: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tag_aggregate(distinct_on: id, where: {tigertree_id: {_is_null: false}, _or: [{data: {_cast: {String: {_ilike: $tag}}}}, {tagtype: {name: {_ilike: $tag}}}], tigertree: {host: {_ilike: $host}, tree_type: {_eq: "file"}}}) {
      aggregate {
        count
      }
    }
    tag(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {tigertree_id: {_is_null: false}, _or: [{data: {_cast: {String: {_ilike: $tag}}}}, {tagtype: {name: {_ilike: $tag}}}], tigertree: {host: {_ilike: $host}, tree_type: {_eq: "file"}}}) {
      tigertree {
        ...tigertreeData
      }
...
export function SearchTabFilesLabel(props) {
    return (
        <tigerSearchTabLabel label={"Files"} iconComponent={<AttachmentIcon/>} {...props}/>
    )
}
...
const SearchTabFilesSearchPanel = (props) => {
    ...
    return (
        <Grid container spacing={1} style={{paddingTop: "10px", paddingLeft: "10px", maxWidth: "100%"}}>
            <Grid item xs={2}>
                <tigerTextField placeholder="Host Name Search..." value={searchHost}
            ...
            <Grid item xs={2}>
                <Button variant="contained" color="primary" component="label">Host File in tiger <input
...
export const SearchTabFilesPanel = (props) => {
    ...
    const handleFileBrowserSearchResults = (data) => {
        ...
        if (searchField === "Tag") {
            setTotalCount(data?.tag_aggregate?.aggregate?.count || 0);
            setFileBrowserData(data?.tag?.map(t => t.tigertree) || []);
        } else {
            setTotalCount(data.tigertree_aggregate.aggregate.count);
            setFileBrowserData(data.tigertree);
        }
