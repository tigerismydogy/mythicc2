...
query nameProcessQuery($pid: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tigertree_aggregate(distinct_on: id, where: {full_path_text: {_ilike: $pid}, host: {_ilike: $host}, tree_type: {_eq: "process"}}) {
      aggregate {
        count
      }
    }
    tigertree(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {host: {_ilike: $host}, full_path_text: {_ilike: $pid}, tree_type: {_eq: "process"}}) {
      ...tigertreeData
    }
  }
`;
...
query nameProcessQuery($name: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tigertree_aggregate(distinct_on: id, where: {name_text: {_ilike: $name}, host: {_ilike: $host}, tree_type: {_eq: "process"}}) {
      aggregate {
        count
      }
    }
    tigertree(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {host: {_ilike: $host}, name_text: {_ilike: $name}, tree_type: {_eq: "process"}}) {
      ...tigertreeData
    }
  }
`;
...
query tagProcessQuery($tag: String!, $host: String!, $offset: Int!, $fetchLimit: Int!) {
    tag_aggregate(distinct_on: id, where: {tigertree_id: {_is_null: false}, _or: [{data: {_cast: {String: {_ilike: $tag}}}}, {tagtype: {name: {_ilike: $tag}}}], tigertree: {host: {_ilike: $host}, tree_type: {_eq: "process"}}}) {
      aggregate {
        count
      }
    }
    tag(limit: $fetchLimit, distinct_on: id, offset: $offset, order_by: {id: desc}, where: {tigertree_id: {_is_null: false}, _or: [{data: {_cast: {String: {_ilike: $tag}}}}, {tagtype: {name: {_ilike: $tag}}}], tigertree: {host: {_ilike: $host}, tree_type: {_eq: "process"}}}) {
      tigertree {
        ...tigertreeData
      }
...
export function SearchTabProcessesLabel(props){
    return (
        <tigerSearchTabLabel label={"Processes"} iconComponent={<AccountTreeIcon />} {...props}/>
    )
}
...
export const SearchTabProcessPanel = (props) =>{
    ...
    const handleKeylogSearchResults = (data) => {
        ...
        if(searchField === "Tag"){
            ...
            setProcessData(data?.tag?.map(t => t.tigertree) || []);
        } else {
            setTotalCount(data.tigertree_aggregate.aggregate.count);
            setProcessData(data.tigertree);
        }
