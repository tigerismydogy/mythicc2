...
export function ExpandedCallbackSideDetailsTable(props){
    ...
    const lastCheckinTimestampFromtiger = React.useRef("");
    ...
    const updateDisplayTime = () => {
        ...
        if(newTimeDifference.includes("m")){
            if(newTimeDifference.includes("m0s")){
                lastCheckinTimestamp.current = toLocalTime(lastCheckinTimestampFromtiger.current, false);
    ...
    useInterval( () => {
        ...
        let currenttiger = new Date(props.current_time);
        let timeskew = (new Date()) - currenttiger;
        lastCheckinDifference.current = last - timeskew;
        lastCheckinTimestampFromtiger.current = props.last_checkin;
        ...
        <Table  size="small" style={{"overflow": "scroll"}}>
                <TableBody style={{whiteSpace: "pre"}}>
                    ...
                    <TableRow hover>
                        ...
                        <TableCell>
                            <tigerStyledTooltip title={lastCheckinTimestamp.current} >
                                {displayTime}
                            </tigerStyledTooltip>
                        </TableCell>
                    </TableRow>
                    ...
                    <tigerDialog fullWidth={true} maxWidth="lg" open={openC2Dialog}
                    ...
                    <tigerDialog fullWidth={true} open={openEditDescriptionDialog}  onClose={() => {setOpenEditDescriptionDialog(false);}}
                        innerDialog={
                            <tigerModifyStringDialog title={"Edit Callback's Description"} onClose={() => {setOpenEditDescriptionDialog(false);}} value={props.description} onSubmit={editDescriptionSubmit} />
                        }
                        />
                    <TableRow hover>
                    ...
                    <TableRow hover>
                        ...
                        <TableCell>{props.tigertree_groups.join(", ")}</TableCell>
                    </TableRow>
