...
import {usetigerSetting} from "../../tigerComponents/tigerSavedUserSetting";
...
export function SettingsOperatorUIConfigDialog(props) {
    ...
    const initialShowMediaValue = usetigerSetting({setting_name: "showMedia", default_value: "true"});
    const [showMedia, setShowMedia] = React.useState(initialShowMediaValue);

    const initialHideUsernameValue = usetigerSetting({setting_name: "hideUsernames", default_value: "false"});
    const [hideUsernames, setHideUsernames] = React.useState(initialHideUsernameValue);

    const initialShowIPValue = usetigerSetting({setting_name: "showIP", default_value: "false"});
    const [showIP, setShowIP] = React.useState(initialShowIPValue);

    const initialShowHostnameValue = usetigerSetting({setting_name: "showHostname", default_value: "false"});
    const [showHostname, setShowHostname] = React.useState(initialShowHostnameValue);

    const initialShowCallbackGroupsValue = usetigerSetting({setting_name: "showCallbackGroups", default_value: "false"});
          ...
          <Table size="small" style={{ "maxWidth": "100%", "overflow": "scroll"}}>
              <TableBody>
                <TableRow hover>
                  ...
                  <TableCell>
                    <tigerTextField type="number" value={fontSize} onChange={onChangeFontSize} showLabel={false} />
                  </TableCell>
                </TableRow>
                <TableRow hover>
                  <TableCell>Font Family</TableCell>
                  <TableCell>
                  <tigerTextField value={fontFamily} onChange={onChangeFontFamily} showLabel={false} multiline maxRows={5} />
                  </TableCell>
                </TableRow>
                <TableRow hover>
