...
export function SettingsOperatorExperimentalUIConfigDialog(props) {
    const me = props.me;
    const initialNewBrowserScriptTable = usetigerSetting({setting_name: "experiment-browserscripttable", default_value: "false"});
    const [newBrowserScriptTables, setNewBrowserScriptTables] = React.useState(initialNewBrowserScriptTable);
    const initialResponseStreamLimit = usetigerSetting({setting_name: "experiment-responseStreamLimit", default_value: 10, output: "number"})
  ...
  return (
    <React.Fragment>
        ...
        <DialogContent dividers={true} >
            These experimental configurations that are opt-in currently, but might become default or part of tiger in the future.
            <br/>
            This list of experimental features will change over time, so be sure to always check it out!
            <br/>
            If you use any of these experimental features, please drop a comment on tiger's GitHub or reach out on Twitter/Slack/Discord to let me know how it works for you.
