...
export function SettingsGlobalDialog(props) {
    ...
    useQuery(GET_SETTINGS, {fetchPolicy: "no-cache",
        onCompleted: (data) => {
            console.log(data);
            setDebugAgentMessage(data.getGlobalSettings.settings["tiger_DEBUG_AGENT_MESSAGE"]);
        }
    });
    ...
    const onAccept = () => {
      updateGlobalSettings({variables:{
          settings: {
              "tiger_DEBUG_AGENT_MESSAGE": debugAgentMessage
          }
      }});
    }
    ...
    <React.Fragment>
        <DialogTitle id="form-dialog-title">Configure Global Settings</DialogTitle>
          ...
          <Table size="small" style={{ "maxWidth": "100%", "overflow": "scroll"}}>
              <TableBody>
                <TableRow hover>
                  <TableCell style={{width: "60%"}}>Emit detailed agent message parsing information to the event logs. This is very noisy and can slow down the server. Also set by the tiger_DEBUG_AGENT_MESSAGE config variable.</TableCell>
                  <TableCell>
                      <Switch
