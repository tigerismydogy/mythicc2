...
import {tigerFileContext} from "../../tigerComponents/tigerFileContext";
...
export const ParseForDisplay = ({cmd, filename}) => {
    ...
    return (
        cmd.parameter_type === "File" ? (
            <tigerFileContext agent_file_id={cmd.value} display_link={""} />
        ) : (renderObj)
    )
}
...
function DetailedPayloadInnerTable(props){
    ...
    const { loading, error, data } = useQuery(GET_Payload_Details, {
        ...
        onCompleted: data => {
            ...
            { 
                return {cmd: c.command.cmd, tiger: c.command.version, payload: c.version} 
            ...
            <Table size="small" aria-label="details" style={{ "overflowWrap": "break-word"}}>
              ...
              <Table size="small" aria-label="details" style={{ "overflowWrap": "break-word"}}>
                    ...
                    {commands.length > 0 &&
                      ...
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell>tiger Version</TableCell>
                      ...
                      <TableBody>
                        {
                          commands.map( (cmd) => (
                              <TableRow key={cmd.cmd + props.payload_id} hover style={{background: cmd.tiger > cmd.payload ? (theme.palette.error.main) : ("")}}>
                                  <TableCell>{cmd.cmd}</TableCell>
                                  <TableCell>{cmd.tiger}</TableCell>
                ...
                {openAddRemoveCommandsDialog &&
                  <tigerDialog fullWidth={true} maxWidth="md" open={openAddRemoveCommandsDialog} 
