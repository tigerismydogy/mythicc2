...
const fileDataFragment = gql`
    fragment fileObjData on tigertree {
        comment
        deleted
        task_id
        filemeta {
...
const rootFileQuery = gql`
    ...
    query myRootFolderQuery {
        tigertree(where: { parent_path_text: { _eq: "" }, tree_type: {_eq: "file"} }) {
            ...fileObjData
        }
    }
`;
           ...
           for(let i = 0; i < data.tigertree.length; i++){
               let currentGroups = data.tigertree[i]?.["callback"]?.["tigertree_groups"] || ["Unknown Callbacks"];
                for(let j = 0; j < currentGroups.length; j++){
                    ...
                    treeRootDataRef.current[currentGroups[j]][data.tigertree[i]["host"]][data.tigertree[i]["full_path_text"]] = {...data.tigertree[i]}
                }
           }
        ...
        onData: ({data}) => {
            for(let i = 0; i < data.data.tigertree_stream.length; i++){
                ...
                for(let j = 0; j < currentGroups.length; j++) {
                    ...
                    treeRootDataRef.current[currentGroups[j]][data.data.tigertree_stream[i]["host"]][data.data.tigertree_stream[i]["full_path_text"]] = {...data.data.tigertree_stream[i]};
                }
            }
...
const FileBrowserTableTop = ({
    ...
    useEffect(() => {
        ...
        const groups = selectedFolderData?.callback?.tigertree_groups?.join(", ") || "";
        if(groups.length > 0 ){
            ...
            <Grid item xs={12}>
                <tigerTextField
                    ...
                    InputProps={{
                        endAdornment: (
                            <React.Fragment>
                                <tigerStyledTooltip title={`Task current callback (${tabInfo["displayID"]}) to list contents`}>
                                    <IconButton style={{ padding: '3px' }} onClick={onLocalListFilesButton} size="large">
                                        <RefreshIcon color='info' />
                                    </IconButton>
                                </tigerStyledTooltip>
                                <tigerStyledTooltip title={`Upload file to folder via current callback (${tabInfo["displayID"]})`}>
                                    <IconButton style={{ padding: '3px' }} onClick={onLocalUploadFileButton} size="large">
                                        <CloudUploadIcon color="info" />
                                    </IconButton>
                                </tigerStyledTooltip>
                                <tigerStyledTooltip title={showDeletedFiles ? 'Hide Deleted Files' : 'Show Deleted Files'}>
                                    <IconButton
                                        style={{ padding: '3px' }}
