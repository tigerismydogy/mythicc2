...
import {ViewEditTagsDialog} from '../../tigerComponents/tigerTag';
...
const SpeedDialDisplayGeneric = ({toggleViewBrowserScript, toggleSelectAllOutput,
  ...
  const [reissueTask] = useMutation(ReissueTaskMutationGQL, {
    onCompleted: data => {
      if(data.reissue_task.status === "success"){
        snackActions.success("Successfully re-issued task to tiger");
      }else{
        snackActions.error("Failed to re-issue task to tiger: " + data.reissue_task.error);
      }
    },
    onError: data => {
      console.log(data);
  ...
  return (
      <React.Fragment>
        ...
        {openTaskTagDialog ?
            (<tigerDialog fullWidth={true} maxWidth="md" open={openTaskTagDialog}
        ...
        {openOpsecDialog.open ?
            (<tigerDialog fullWidth={true} maxWidth="md" open={openOpsecDialog.open}
        ...
        {openStdoutStderrDialog ?
            (<tigerDialog fullWidth={true} maxWidth="md" open={openStdoutStderrDialog}
