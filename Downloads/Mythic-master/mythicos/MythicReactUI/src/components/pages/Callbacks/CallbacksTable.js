    ...
    const columns = useMemo( 
      () => 
        [
          ...
          {key: "tigertree_groups", type: 'array', name: "Groups"},
                ...
                return [...prev, columns.map( c => {
                    switch(c.name){
                        case "Interact":
                            ...
                            />;
                        case "Groups":
                            return <CallbacksTableStringCell key={`callback${row.id}_${c.name}`} cellData={row.tigertree_groups.join(", ")} />;
                        case "IP":
...
const accessorFn = (row, h) => {
    ...
    if(h.name === "Groups"){
        return row.tigertree_groups.join(", ");
    }
    if(h.name === "IP"){
        try{
...
function CallbacksTableMaterialReactTablePreMemo(props){
    ...
    const columnFields = [
        ...
        {key: "tigertree_groups", type: 'array', name: "Groups", enableHiding: true},
    ...
    const localCellRender = React.useCallback( ({cell, h}) => {
        ...
        switch(h.name){
            case "Interact":
                ...
                />;
            case "Groups":
                return <CallbacksTableStringCell cellData={row.tigertree_groups.join(", ")} />;
            case "IP":
