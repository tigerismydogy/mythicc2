...
export const SearchBar = ({onSubmitSearch}) => {
  ...
  return (
    <div style={{marginTop: "10px"}}>
      <tigerTextField value={search} autoFocus onEnter={onSubmitLocalSearch} onChange={(n,v,e) => setSearch(v)} placeholder="Search All Output of This Task" name="Search..."
        InputProps={{
          endAdornment: 
          <React.Fragment>
              <tigerStyledTooltip title="Search">
...
const ResponseDisplayComponent = ({rawResponses, viewBrowserScript, output, command_id, task, search, expand}) => {
  ...
  const useNewBrowserScriptTable = usetigerSetting({setting_name: "experiment-browserscripttable", default_value: "false"});
